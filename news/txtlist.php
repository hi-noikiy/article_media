...
$getstr=$_GET[str];
$id=returnsx("i");
while0("*","epzhu_news where id=".$id);if(!$row=mysql_fetch_array($res)){php_toheader(weburl."404.html");exit;}

$sj=date("Y-m-d H:i:s");
while1("*","epzhu_news where type1id=".$row[type1id]." and type2id=".$row[type2id]." and zt=0 and lastsj>='".$row[lastsj]."' and id<>".$row[id]." order by sj asc");
if($row1=mysql_fetch_array($res1)){$pre="<a href='txtlist_i".$row1[id]."v.html'>".$row1[tit]."</a>";}
else{$pre="已是第一篇内容";}
while1("*","epzhu_news where type1id=".$row[type1id]." and type2id=".$row[type2id]." and zt=0 and lastsj<='".$row[lastsj]."' and id<>".$row[id]." order by sj desc");
if($row1=mysql_fetch_array($res1)){$nex="<a href='txtlist_i".$row1[id]."v.html'>".$row1[tit]."</a>";}
else{$nex="已是最后一篇内容";}
checkdjl("c2",$row[id],"epzhu_news");
...
<div class="newsmain">
 ...
 <!--评论B-->
 <? $ses="epzhu_newspj where newsbh='".$row[bh]."'";?>
 ...
 <? while1("*","epzhu_news where zt=0 order by lastsj desc limit 10");while($row1=mysql_fetch_array($res1)){?>
