...
$id=$_GET[id];
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_task where id=".$id);if(!$row=mysql_fetch_array($res)){php_toheader("./");}
$bh=$row[bh];
taskok($row["id"]);

$sqluser="select * from epzhu_user where id=".$row[userid]."";mysql_query("SET NAMES 'GBK'");$resuser=mysql_query($sqluser);$rowuser=mysql_fetch_array($resuser);
$userid=0;
$xgnum=0;
if(!empty($_SESSION[SHOPUSER])){
$sqluserM="select * from epzhu_user where uid='".$_SESSION[SHOPUSER]."'";mysql_query("SET NAMES 'GBK'");$resuserM=mysql_query($sqluserM);$rowuserM=mysql_fetch_array($resuserM);
$userid=$rowuserM[id];
while1("*","epzhu_taskhf where bh='".$bh."' and useridhf=".$userid."");if($row1=mysql_fetch_array($res1)){$xgnum=$row1[xgnum];$mybh=$row1[mybh];$myid=$row1[id];$mymoney=$row1[money1];$mytxt=$row1[txt];}
}
...
if($_GET[control]=="add"){ //单人任务
 ...
 if(panduan("*","epzhu_taskhf where bh='".$row[bh]."' and useridhf=".$userid."")==0){
 $mybh=time()."t".$userid;
 intotable("epzhu_taskhf","mybh,bh,uip,userid,useridhf,sj,txt,money1,ifxz,xgnum,taskty","'".$mybh."','".$row[bh]."','".$uip."',".$row[userid].",".$userid.",'".$sj."','".$txt."',".$money1.",0,1,0");
 }else{
 updatetable("epzhu_taskhf","money1=".$money1.",txt='".$txt."',xgnum=xgnum+1 where bh='".$row[bh]."' and useridhf=".$userid."");
 }
 ...
 if(panduan("*","epzhu_taskhf where bh='".$row[bh]."' and useridhf=".$userid." and (zt=0 or zt=1 or zt=3 or zt=4)")==1){Audit_alert("你有任务未完成，不能再次接单！","view".$id.".html");}
 $txt=sqlzhuru($_POST[s1]);
 $mybh=time()."t".$userid;
 $money1=$row[money1]/$row[tasknum];
 $rwdq=date("Y-m-d H:i:s",strtotime("+".$row[rwzq]." day"));
 intotable("epzhu_taskhf","mybh,bh,uip,userid,useridhf,sj,txt,money1,ifxz,xgnum,taskty,zt,zbsj,rwdq","'".$mybh."','".$row[bh]."','".$uip."',".$row[userid].",".$userid.",'".$sj."','".$txt."',".$money1.",0,1,1,0,'".$sj."','".$rwdq."'");
 $uf=$row[useridhf]."yj".$userid."yj";
 updatetable("epzhu_task","useridhf='".$uf."' where id=".$id);
 $txt="接手成功，开始做任务，须在".$rwdq."前完成任务，并提交验收";
 intotable("epzhu_tasklog","bh,userid,useridhf,admin,txt,sj,fj","'".$bh."',".$row[userid].",".$userid.",2,'".$txt."','".$sj."',''");
...
<div class="yjcode">
 ...
 <div class="vleft">
  ...
  <div class="jbzl">
   ...
   <? }elseif(1==$row[qqxs] && returncount("epzhu_taskhf where bh='".$row[bh]."' and useridhf=".$userid."")>0){?>
   <a class="a3" target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<?=$rowuser[uqq]?>&site=<?=weburl?>&menu=yes"><?=$rowuser[uqq]?></a>
   <? }elseif(2==$row[qqxs] && returncount("epzhu_taskhf where bh='".$row[bh]."' and ifxz=1 and useridhf=".$userid."")>0){?>
   <a class="a3" target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<?=$rowuser[uqq]?>&site=<?=weburl?>&menu=yes"><?=$rowuser[uqq]?></a>
   ...
   <? }elseif(1==$row[motxs] && returncount("epzhu_taskhf where bh='".$row[bh]."' and useridhf=".$userid."")>0){?>
   <a class="s2" href="javascript:void(0);"><?=$rowuser[mot]?></a>
   <? }elseif(2==$row[motxs] && returncount("epzhu_taskhf where bh='".$row[bh]."' and ifxz=1 and useridhf=".$userid."")>0){?>
   <a class="s2" href="javascript:void(0);"><?=$rowuser[mot]?></a>
   <? }elseif($userid==$row[userid]){?>
   <a class="s2" href="javascript:void(0);"><?=$rowuser[mot]?></a>
   <? }else{?>
   ...
   <? $bmnum=returncount("epzhu_taskhf where bh='".$row[bh]."'");?>
   <? if(0==$row[zt] && $bmnum==0){?>
   <ul class="ztu">
  ...
  <? if($userid==$row[userid]){$cy=returncount("epzhu_taskhf where bh='".$row[bh]."'");?>
  <? if($cy==0){?>
  <div class="jisuan">
  ...
  <?
  $zh=returnsum("money1","epzhu_taskhf where bh='".$row[bh]."'");
  while1("*","epzhu_taskhf where bh='".$bh."' order by money1 desc");$row1=mysql_fetch_array($res1);$moneyg=$row1[money1];
  while1("*","epzhu_taskhf where bh='".$bh."' order by money1 asc");$row1=mysql_fetch_array($res1);$moneyd=$row1[money1];
  ?>
  ...
  <!--多人任务开始-->
  <? if($row[taskty]==1 && $userid!=$row[userid] && panduan("*","epzhu_taskhf where bh='".$row[bh]."' and useridhf=".$userid." and (zt=0 or zt=1 or zt=3 or zt=4)")==0){?>
  <div class="baojia">
  ...
  <?
  while1("*","epzhu_taskhf where bh='".$bh."' order by sj desc");while($row1=mysql_fetch_array($res1)){
  while2("*","epzhu_user where id=".$row1[useridhf]);$row2=mysql_fetch_array($res2);
  ?>
  <a name="tb<?=$row1[id]?>"></a>
  <div class="taskhf">
   <ul class="u1">
