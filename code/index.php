...
<style>
	...
	<div class="head-nav">
		<div class="main">
				<div class="nav-link">
					<li class="left">
						<a href="/">首页</a><a href="/code/search_j37v.html" class="bold cur">源码集市</a><a href="/serve/search_j208v.html" class="bold  ">服务市场</a><a href="/web/search_j213v.html" class="bold ">网站寄售</a><a href="/productym/search_j236v.html" class="">域名交易</a><a href="/task/" class="">任务大厅</a>					</li>
					<li class="right">
 ...
 ?>
  ...
  <? while1("*","epzhu_user where id=".$row[userid]);$row1=mysql_fetch_array($res1);$xy=returnjgdw($row1[xinyong],"",returnxy($row[userid],1));?>
	...
	<? if($row[fhxs]==2){?><a  class='tips' href="javascript:;"  T-bg='#b68571' title="自动发货商品，拍下后，即可收到来自该商品的发货（下载）链接" color="#ffa800"><i class=send>自</i></a><? }?>
														    ...
														    <? while1("*","epzhu_user where id=".$row[userid]);$row1=mysql_fetch_array($res1);$xy=returnjgdw($row1[xinyong],"",returnxy($row[userid],1));?>
	<? if($row[azsf]==1){?><a T-bg='#999' href='javascript:void();' onclick='anzhuang(1,<?=$row[id]?>)' title='收费安装(<?=$row[anzhuang]?>),点击查看详细安装要求' class='installing tips'><i class='install200'>安</i></a><? }?>
														
														<? while1("*","epzhu_user where id=".$row[userid]);$row1=mysql_fetch_array($res1);$xy=returnjgdw($row1[xinyong],"",returnxy($row[userid],1));?>
	<? if($row[azsf]==0){?><a T-bg='#71a3f5' href='javascript:void();' onclick='anzhuang(1,<?=$row[id]?>)'  title='免费安装,点击查看详细安装要求' class='installing tips'><i class='install0'>安</i></a><? }?>
 ...
 ?>
						...
						<li>
								<a href="<?=$au?>" target="_blank" title='<?=strgb2312($row[tit],0,30)?>'>
									<i><?=shigtypeid($row['tysx'])?></i><b>￥<?=$money1?></b> 
