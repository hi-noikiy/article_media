...
<div class="uk box">
 ...
 <div class="d2"><input class="inp" name="tyhsj1" placeholder="请设置优惠开始时间" value="<?=$row[yhsj1]?>" readonly onClick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" type="text"/></div>
</div>
<div class="uk box">
 <div class="d1">优惠截止<span class="s1"></span></div>
 <div class="d2"><input class="inp" name="tyhsj2" placeholder="请设置优惠截止时间" value="<?=$row[yhsj2]?>" readonly onClick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" type="text"/></div>
</div>
</div>
...
<div class="uk box">
 ...
 <option value="1" <? if(1==$row[fhxs]){?>option="option"<? }?>>手动发货</option>
 <? }?>
 <? if(strstr($rowcontrol[fhxs],"2") || empty($rowcontrol[fhxs])){?>
 <option value="2" <? if(2==$row[fhxs]){?>option="option"<? }?>>网盘下载</option>
 <? }?>
 <? if(strstr($rowcontrol[fhxs],"3") || empty($rowcontrol[fhxs])){?>
 <option value="3" <? if(3==$row[fhxs]){?>option="option"<? }?>>网站直接下载</option>
 <? }?>
 <? if(strstr($rowcontrol[fhxs],"4") || empty($rowcontrol[fhxs])){?>
 <option value="4" <? if(4==$row[fhxs]){?>option="option"<? }?>>点卡交易</option>
 <? }?>
 <? if(strstr($rowcontrol[fhxs],"5") || empty($rowcontrol[fhxs])){?>
 <option value="5" <? if(5==$row[fhxs]){?>option="option"<? }?>>实物快递</option>
 <? }?>
 </select>
 </div>
 <div class="d3"><img src="../img/rightjian.png" height="13" /></div>
...
<div class="uk box">
 ...
 <div class="d21">【<a href="../upload/<?=$row[userid]?>/<?=$row[bh]?>/<?=$row[upf]?>" class="blue" target="_blank">点击预览</a>】</div>
</div>
<? }?>
</div>
...
<div class="uk box">
 ...
 <div class="d21"><strong class="red"><?=$row[kcnum]?>件</strong> [<a href="kclist.php?bh=<?=$bh?>" target="_blank" class="blue">管理库存</a>]</div>
</div>
<div class="uk box">
...
<div class="uk box">
 <div class="d1">商品详情<span class="s1"></span></div>
 <div class="d2"><textarea placeholder="请输入商品详情" name="content" style="height:100px;"><?=$row[txt]?></textarea></div>
</div>
