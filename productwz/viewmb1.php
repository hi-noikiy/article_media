...
<script language="javascript" src="<?=weburl?>productwz/view.js"></script>
...
<tr><td>宝贝数量：<?=returncount("epzhu_prowz where userid=".$rowsell[id]." and zt=0")?> 个</td></tr>
...
<div class="gsbox">
<a href="/shop/prolist_i<?=$rowsell[id]?>v.html">源码<em>(<?=returncount("epzhu_prowz where userid=".$rowsell[id]." and zt=0")?>)</em></em></a></div>
</li>
...
<ul class="c_g_but">
  <li class="l2"><a class="cartadd"  id="bcar" href="javascript:buyInto('<?=$row[bh]?>')"><i class="iconfont"></i> <b>立即购买</b></a>
   ...
   $a1="none";$a2="none";
   if($_SESSION["SHOPUSER"]==""){$a1="";}else{
	if(panduan("probh,userid","epzhu_carwz where probh='".$row[bh]."' and userid=".$nuid)==1){$a2="";}else{$a1="";}
   }
   ?>
   ...
   <a href="../user/carwz.php" id="cara2" style="display:<?=$a2?>;" class="carok" title="已在购物车"><img src="img/cheok.png" width="50" height="40" border="0" /></a></li></ul>
<UL class="c_g_ser">
		<DIV class=fw_name>
...
<p>
 ...
 for($i=0;$i<=count($a);$i++){
  ...
  if($ai!=""){
   if(!is_numeric($ai)){$z1=preg_split("/:/",$ai);$ai=$z1[0];}
   while1("*","epzhu_typesxwz where id=".$ai);if($row1=mysql_fetch_array($res1)){
    while2("*","epzhu_typesxwz where name1='".$row1[name1]."' and admin=1 and ifjd=1");if($row2=mysql_fetch_array($res2)){
     if(!in_array($row1[name1],$sx1arr)){$sx1arr[$m]=$row1[name1];$m++;}
     if(!is_numeric($a[$i])){$z1=preg_split("/:/",$a[$i]);$v=$z1[1];}else{$v=$row1[name2];}
     $sxall=$sxall.$row1[name1].":".$v."xcf";
    }
...
<a class="gopage">商品综合评分<strong class="g_ac0_h"><?=round(($row[pf1]+$row[pf2]+$row[pf3])/3,2)?></strong>分</a>
 ...
 <? 
 while1("*","epzhu_propjwz where probh='".$row[bh]."' order by sj desc limit 10");while($row1=mysql_fetch_array($res1)){
...
<div class="c_l_cap">&nbsp;&nbsp;&nbsp;本店销量榜！</div>
 ...
 <dl class="dropList-hover">
     ...
     while1("*","epzhu_prowz where userid=".$row[userid]." and zt=0 and ifxj=0 order by xsnum desc limit 10");while($row1=mysql_fetch_array($res1)){$tp="../".returntp("bh='".$row1[bh]."' order by xh asc","-2");
	 ?>
...
<table class="table2">
   ...
   $a=preg_split("/xcf/",$row[tysx]);$sx1arr=array();$sxall="xcf";$m=0;for($i=0;$i<=count($a);$i++){
    ...
    while1("*","epzhu_typesxwz where id=".$ai);if($row1=mysql_fetch_array($res1)){if(!in_array($row1[name1],$sx1arr)){$sx1arr[$m]=$row1[name1];$m++;}
