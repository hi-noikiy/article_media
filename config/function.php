...
function uploadtp($tbh,$tty,$tuid){
 ...
 $nxh=returnxh("epzhu_tp"," and bh='".$tbh."' and type1='".$tty."'");
 if(panduan("*","epzhu_tp where bh='".$tbh."' and type1='".$tty."' and iffm=1")==1){$fmv=0;}else{$fmv=1;}
 intotable("epzhu_tp","bh,tp,type1,iffm,sj,userid,xh","'".$tbh."','".$row3[tp]."','".$tty."',".$fmv.",'".$row3[sj]."','".$tuid."',".$nxh."");
...
html_template("tem/tongepzhu.php","../tem/tongepzhu.html");
html_template("tem/userepzhucom.php","../tem/userepzhucom.html");
...
deletetable("epzhu_pro where zt=99 and sj<='".$nsj."'");
deletetable("epzhu_protype where zt=99 and sj<='".$nsj."'");
...
function delproduct($b,$u){
 ...
 deletetable("epzhu_propj where probh='".$b."'");
 ...
 deletetable("epzhu_prouserdj where probh='".$b."'");
 deletetable("epzhu_provideo where probh='".$b."'");
 delDirAndFile("../upload/".$u."/".$b."/");
 deletetable("epzhu_pro where bh='".$b."'");
}
...
if(is_numeric($userid)){
 ...
 while3("bh,userid","epzhu_pro where userid=".$userid);while($row3=mysql_fetch_array($res3)){delproduct($row3[bh],$row3[userid]);}
 deletetable("epzhu_protype where userid=".$userid);
 deletetable("epzhu_shopmenu where userid=".$userid);
 deletetable("epzhu_propj where userid=".$userid);
 ...
 deletetable("epzhu_prouserdj where userid=".$userid);
...
function kamikc($b){
 if(!empty($b)){
  if(panduan("bh,fhxs","epzhu_pro where bh='".$b."' and fhxs=4")==1){
  $a=returncount("epzhu_kc where ifok=0 and probh='".$b."'");
  updatetable("epzhu_pro","kcnum=".$a." where bh='".$b."'");
  }
 }
}
...
//获取商品分类
function shigtypeid($t1){
	$arr_slideshow = explode('xcf',$t1);
	foreach($arr_slideshow as $value){
		if($value!=''){
		$sql='select * from epzhu_typesx where id='.$value.'';
