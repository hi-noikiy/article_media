...
if(empty($aid)){$aid=0;}
$sqlp="select * from epzhu_typekf where id=".$aid;mysql_query("SET NAMES 'GBK'");$resp=mysql_query($sqlp);
...
if(empty($aid)){$aid=0;}
$sqlp="select * from epzhu_protypekf where id=".$aid;mysql_query("SET NAMES 'GBK'");$resp=mysql_query($sqlp);
 if($rowp=mysql_fetch_array($resp)){
...
$sqlother="select id,sellbl from epzhu_user where id=".$u;mysql_query("SET NAMES 'GBK'");$resother=mysql_query($sqlother);$rowother=mysql_fetch_array($resother);
 if(!empty($rowother[sellbl])){$sbl=$rowother[sellbl];}else{
  $sqlt1="select bh,ty1id from epzhu_prokf where bh='".$pbh."'";mysql_query("SET NAMES 'GBK'");$rest1=mysql_query($sqlt1);
  if($rowt1=mysql_fetch_array($rest1)){
   $sqlt2="select id,sellbl from epzhu_typekf where id=".$rowt1[ty1id];mysql_query("SET NAMES 'GBK'");$rest2=mysql_query($sqlt2);
...
$sqlother="select id,bh from epzhu_prokf where bh='".$b."'";mysql_query("SET NAMES 'GBK'");$resother=mysql_query($sqlother);
$rowother=mysql_fetch_array($resother);
return $rowother[id];
}

function returnxy($u,$t){ //1卖家 2买家
if(1==$t){$sqlxy="select count(*) as id from epzhu_orderkf where selluserid=".$u." and ddzt='suc'";}
elseif(2==$t){$sqlxy="select count(*) as id from epzhu_orderkf where userid=".$u." and ddzt='suc'";}
...
function returnyunfei($u,$s,$sl,$p){//u商家 s买家收货ID sl数量 p商品编号
 ...
 $sqlp="select * from epzhu_prokf where bh='".$p."'";mysql_query("SET NAMES 'GBK'");$resp=mysql_query($sqlp);$rowp=mysql_fetch_array($resp);
 if(5==$rowp[fhxs]){
  $zz=$rowp[zl]*$sl;//总量
...
function returntjmoney($pbh){
 ...
 $sqlt1="select bh,ty1id from epzhu_prokf where bh='".$pbh."'";mysql_query("SET NAMES 'GBK'");$rest1=mysql_query($sqlt1);
 if($rowt1=mysql_fetch_array($rest1)){
  $sqlt2="select id,tjmoney from epzhu_typekf where id=".$rowt1[ty1id];mysql_query("SET NAMES 'GBK'");$rest2=mysql_query($sqlt2);
  if($rowt2=mysql_fetch_array($rest2)){
