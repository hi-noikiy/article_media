<div class="helpleft">
 ...
 <?
 $i=1;
 while1("*","epzhu_helptype where admin=1 order by xh asc");while($row1=mysql_fetch_array($res1)){
 $aurl="search_j".$row1[id]."v.html";
 if(returncount("epzhu_help where ty1id=".$row1[id])==1){
 while3("id,ty1id","epzhu_help where ty1id=".$row1[id]." order by sj desc");$row3=mysql_fetch_array($res3);
 $aurl="view".$row3[id].".html";
 }
 ?>
 <li class="l1"><a href="<?=$aurl?>"><?=$row1[name1]?></a></li>
 <?
 while2("*","epzhu_helptype where admin=2 and name1='".$row1[name1]."' order by xh asc");while($row2=mysql_fetch_array($res2)){
