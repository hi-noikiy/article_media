...
while0("*","epzhu_help where id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("./");}

while1("*","epzhu_help where sj>='".$row[sj]."' and id<>".$row[id]." order by sj asc");
if($row1=mysql_fetch_array($res1)){$pre="<a href='view".$row1[id].".html'>".$row1[tit]."</a>";}else{$pre="已是该分组下第一篇";}

while1("*","epzhu_help where sj<='".$row[sj]."' and id<>".$row[id]." order by sj desc");
