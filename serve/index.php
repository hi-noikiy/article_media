...
<a href="<?=rentser('d','','','','search');?>" class="">自动发货</a>
 <? }?></cite>
		...
		<a href="/serve/search_j208v.html" class="icons del_screen">清除筛选</a></div> <? }?>
 ...
 <? }?>
  ...
  ?><? while1("*","epzhu_user where id=".$row[userid]);$row1=mysql_fetch_array($res1);$xy=returnjgdw($row1[xinyong],"",returnxy($row[userid],1));?>
						<li class='info'>
							<span>
							<a href="/ishop<?=$row1[id]?>/" class="list_seller tips tipso_style" target="_blank" t-fc="#333" t-bg="#fff" t-bc="#fff" t-bs="0 0 16px 0 rgba(53,53,53,.2)" t-w="246px"><img title="<?=returntitdian($row1[shopname],18)?>" src="../upload/<?=$row[userid]?>/shop.jpg" height="18" width="18"><img class="xy" src="../img/dj/<?=returnxytp($xy)?>" title="信用值<?=$xy?>"></a>
							</span>
