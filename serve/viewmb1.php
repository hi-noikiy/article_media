...
<tr><td>宝贝数量：<?=returncount("epzhu_prokf where userid=".$rowsell[id]." and zt=0")?> 个</td></tr>
...
<div class="gsbox">
<a href="/shop/prolist_i<?=$rowsell[id]?>v.html">源码<em>(<?=returncount("epzhu_prokf where userid=".$rowsell[id]." and zt=0")?>)</em></em></a></div>
</li>
...
<ul class="c_g_but">
  <li class="l2"><a class="cartadd"  id="bcar" href="javascript:buyInto('<?=$row[bh]?>')"><i class="iconfont"></i> <b>立即购买</b></a>
   ...
   <a href="../user/carkf.php" id="cara2" style="display:<?=$a2?>;" class="carok" title="已在购物车"><img src="img/cheok.png" width="50" height="40" border="0" /></a></li></ul>
<UL class="c_g_ser">
		<DIV class=fw_name>
...
<table class="table2">
   ...
   $a=preg_split("/xcf/",$row[tysx]);$sx1arr=array();$sxall="xcf";$m=0;for($i=0;$i<=count($a);$i++){
    ...
    while1("*","epzhu_typesxkf where id=".$ai);if($row1=mysql_fetch_array($res1)){if(!in_array($row1[name1],$sx1arr)){$sx1arr[$m]=$row1[name1];$m++;}
