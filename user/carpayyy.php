<?
include("../config/conn.php");//二-次开发--联系QQ:120--036745
include("../config/functionyy.php");//二次---开发--联系QQ:120----036745
sesCheck();
...
$sj=date("Y-m-d H:i:s");
include("buycheckyy.php");

//B
if(sqlzhuru($_POST[jvs])=="carpay" && sqlzhuru($_POST[R1])=="alipay"){
if($needmoney<=$usermoney){Audit_alert("您的可用余额充足，请用余额直接支付。","carpayyy.php?carid=".$carid);}
zwzr();
$bh=time();
$uip=$_SERVER["REMOTE_ADDR"];
$ddbh=time()."|".$rowuser[id];	
$money1=sprintf("%.2f",$needmoney-$usermoney);
intotable("epzhu_dingdang","bh,ddbh,userid,sj,uip,money1,ddzt,alipayzt,bz,ifok,carid","'".$bh."','".$ddbh."',".$rowuser[id].",'".$sj."','".$uip."',".$money1.",'等待买家付款','','',0,'".$caridarr."'");

require_once("alipay.config.php");
$payment_type = "1";
$notify_url = weburl."user/notify_carpay.php"; //服务器异步通知页面路径
$return_url = weburl."user/orderyy.php";//页面跳转同步通知页面路径
...
echo $html_text;exit;
 ...
 if($needmoney>$usermoney){Audit_alert("您的可用余额不足，返回重试。","carpayyy.php?carid=".$carid);}
 zwzr();
 include("buyyy.php");
 php_toheader("orderyy.php");
...
function carpaytj(x){
 ...
 tjwait();
 if(rv=="alipay" || rv==""){fu="carpayyy.php?carid="+x;}
 else if(rv=="tenpay"){fu="tenpay/buy_indexyy.php?carid="+x;}
...
<div class="right">
 ...
 <div class="carpay">
  ...
  <li class="l3">
  <a href="orderyy.php">完成付款</a>
  <a href="javascript:void(0);" onClick="javascript:alert('付款遇到问题？请联系客服');location.href='carpayyy.php?carid=<?=$carid?>';return false;">付款遇到问题</a>
  </li>
  </ul>
  </div>
