<?
 ...
 if($carid[$i]!=""){
  $sqlc="select * from epzhu_carym where id=".$carid[$i];mysql_query("SET NAMES 'GBK'");$resc=mysql_query($sqlc);if($rowc=mysql_fetch_array($resc)){
  $sql="select * from epzhu_proym where bh='".$rowc[probh]."' and zt=0 and ifxj=0";mysql_query("SET NAMES 'GBK'");$res=mysql_query($sql);
  ...
  intotable("epzhu_orderym","probh,sj,uip,selluserid,userid,money1,orderbh,num,tit,ddzt,tcv,buyform,tcid,fhxs,shdz,yunfei,liuyan","'".$row[bh]."','".$sj."','".$uip."',".$row[userid].",".$rowc[userid].",".$rowc[money1].",'".$orderbh."',".$rowc[num].",'".$row[tit]."','wait','".$rowc[tcv]."','".$rowc[buyform]."',".$rowc[tcid].",".$row[fhxs].",'".$rowc[shdz]."',".$rowc[yunfei].",'".$rowc[bz]."'");
  PointUpdateM($rowc[userid],$allmoney*(-1));
  PointIntoM($rowc[userid],"购买商品,数量".$rowc[num],$allmoney*(-1));
  updatetable("epzhu_proym","xsnum=xsnum+".$rowc[num]." where id=".$row[id]);
  
  if(empty($rowc[tcid]) || empty($rowc[tcfhxs])){
  $kc=$row[kcnum]-$rowc[num];updatetable("epzhu_proym","kcnum=".$kc." where id=".$row[id]);
  //无套餐或套餐跟随商品自动发货商品B
  if($row[fhxs]==2 || $row[fhxs]==3 || $row[fhxs]==4){
  updatetable("epzhu_typeym","fhsj='".$sj."',ddzt='db' where ddzt='wait' and orderbh='".$orderbh."'");
  $dbsj=$rowcontrol[dbsj];
  $sqldb="select * from epzhu_typeym where id=".$row[ty1id];mysql_query("SET NAMES 'GBK'");$resdb=mysql_query($sqldb);if($rowdb=mysql_fetch_array($resdb)){
  ...
  $oksj=date("Y-m-d H:i:s",strtotime("+".$dbsj." day"));
  $c_tit="卖家已经发货，款项进入担保阶段，等待买家确认收货";
  intotable("epzhu_mg","money1,sj,selluserid,userid,dboksj,probh,tit,orderbh","".$allmoney.",'".$sj."',".$row[userid].",".$rowc[userid].",'".$oksj."','".$row[bh]."','".$c_tit."','".$orderbh."'");
   //卡密B
   if(4==$rowc[tcfhxs]){
  ...
  updatetable("epzhu_typeym","txt='".$txt."' where orderbh='".$orderbh."'");
  ...
  //写入评价
  $sql2="INSERT INTO `epzhu_propjym` SET `probh`='".$rowc[probh]."',`selluserid`=".$rowm[id].",`userid`=".$rowu[id].",`sj`='".date('Y-m-d H:i:s',time())."',`uip`='127.0.0.1（本机）',`pf1`=5,`pf2`=5,`pf3`=5,`txt`='暂未评价',`orderbh`='".$orderbh."',`pjlx`=1,`iftp`=0;";
  mysql_query($sql2); 
  //写入邮件E
  ...
  deletetable("epzhu_carym where id=".$rowc[id]);
  //////////////////////////////////结束逐一购买
  }
 }	 
 }
