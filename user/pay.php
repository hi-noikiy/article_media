...
intotable("epzhu_dingdang","bh,ddbh,userid,sj,uip,money1,ddzt,alipayzt,bz,ifok","'".$bh."','".$ddbh."',".$userid.",'".$sj."','".$uip."',".$money1.",'等待买家付款','','',0");
...
while1("*","epzhu_paykami where ka='".$tk."' and mi='".$tm."' and ifok=0");if(!$row1=mysql_fetch_array($res1)){Audit_alert("充值失败，卡号或密码不匹配","pay.php");}
$sj=date("Y-m-d H:i:s");
$userid=returnuserid($_SESSION["SHOPUSER"]);
PointIntoM($userid,"卡密直充".$row1[money1]."元",$row1[money1]);
PointUpdateM($userid,$row1[money1]);
updatetable("epzhu_paykami","userid=".$userid.",usesj='".$sj."',ifok=1 where id=".$row1[id]);
