...
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_orderwz where orderbh='".$orderbh."' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderwz.php");}

if(sqlzhuru($_POST[jvs])=="tk"){
 zwzr();
 $zfmm=sha1(sqlzhuru($_POST[t1]));
 if(panduan("uid,zfmm","epzhu_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("安全码有误！","ordertkwz.php?orderbh=".$orderbh);}
 if($row[ddzt]!="db" && $row[ddzt]!="backerr"){Audit_alert("未知错误！","orderviewwz.php?orderbh=".$orderbh);}
 $allmoney=$row[money1]*$row[num]+$row[yunfei];
 updatetable("epzhu_orderwz","ddzt='back',tksj='".$sj."',tkly='".sqlzhuru($_POST[t2])."' where userid=".$userid." and id=".$row[id]);
 $oksj=date("Y-m-d H:i:s",strtotime("+".$rowcontrol[tksj]." day"));
 $sj=date("Y-m-d H:i:s");
 intotable("epzhu_tkkf","money1,sj,tkoksj,selluserid,userid,probh,tkly,orderbh","".$allmoney.",'".$sj."','".$oksj."',".$row[selluserid].",".$row[userid].",'".$row[probh]."','".sqlzhuru($_POST[t2])."','".$orderbh."'");
 while1("*","epzhu_dbkf where orderbh='".$orderbh."' and userid=".$userid);$row1=mysql_fetch_array($res1);
 $dboksj=date("Y-m-d H:i:s",strtotime("+".$rowcontrol[tksj]." day",strtotime($row1[dboksj])));
 updatetable("epzhu_dbkf","dboksj='".$dboksj."' where id=".$row1[id]);
 //通知B
 $sqluser="select id,mot,ifmot,email,ifemail from epzhu_user where id=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$resuser=mysql_query($sqluser);if(!$rowuser=mysql_fetch_array($resuser)){Audit_alert("安全码有误！","ordertkwz.php?orderbh=".$orderbh);}
 if(!empty($rowuser[mot]) && $rowuser[ifmot]==1 && $rowcontrol[ifmob]=="on"){

 while3("*","epzhu_smsmb where mybh='005'");
 ...
 //通知E
 php_toheader("orderviewwz.php?orderbh=".$orderbh); 
...
<div class="right">
 ...
 <? include("ordervwz.php");?>
 
 <? if($row[ddzt]=="db" || $row[ddzt]=="backerr"){?>
 <script language="javascript">
 function tj(){
 if((document.f1.t2.value).replace("/\s/","")==""){layer.alert('请输入您的退款理由', {icon:5});return false;}
 if((document.f1.t1.value).replace("/\s/","")==""){layer.alert('请输入安全码', {icon:5});return false;}
 layer.msg('正在进行退款', {icon: 16  ,time: 0,shade :0.25});
 f1.action="ordertkwz.php?orderbh=<?=$orderbh?>";
 }
 </script>
 ...
 * 卖家也挺不容易，如果是商品存在问题，而卖家又能积极处理问题，您可以<a href="http://wpa.qq.com/msgrd?v=3&uin=<?=returnqq($row[selluserid])?>&site=<?=weburl?>&menu=yes" target=_blank class="blue">与卖家再协商下</a>。
 ...
 <li class="l2">请输入您的安全码：(<a href="zfmmwz.php" class="red">忘了安全码？</a>)</li>
