...
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_orderyy where orderbh='".$orderbh."' and ddzt='backerr' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderyy.php");}

if(sqlzhuru($_POST[jvs])=="jf"){
 zwzr();
 $txt=sqlzhuru1($_POST[content]);
 if(empty($txt)){Audit_alert("申请理由内容不得为空，返回重试！","orderjfyy.php?orderbh=".$orderbh);}
 intotable("epzhu_orderyylogkf","orderbh,userid,selluserid,admin,txt,sj","'".$orderbh."',".$row[userid].",".$row[selluserid].",1,'".$txt."','".$sj."'");
 updatetable("epzhu_orderyy","ddzt='jf' where orderbh='".$orderbh."'");
...
<div class="right">
 ...
 f1.action="orderjfyy.php?orderbh=<?=$orderbh?>";
 }
 </script>
