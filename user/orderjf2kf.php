...
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_orderkf where orderbh='".$orderbh."' and (ddzt='jf' or ddzt='jfbuy' or ddzt='jfsell') and selluserid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("sellorderkf.php");}

if(sqlzhuru($_POST[jvs])=="jf"){
 zwzr();
 $txt=sqlzhuru1($_POST[content]);
 if(empty($txt)){Audit_alert("申请理由内容不得为空，返回重试！","orderjf2kf.php?orderbh=".$orderbh);}
 intotable("epzhu_orderlogkf","orderbh,userid,selluserid,admin,txt,sj","'".$orderbh."',".$row[userid].",".$row[selluserid].",2,'".$txt."','".$sj."'");
 php_toheader("orderjf2kf.php?orderbh=".$orderbh); 
...
<div class="right">
 ...
 <li class="l1"><a href="sellorderkf.php">我的订单</a></li>
 </ul>
 ...
 <div class="jfgtlist">
  ...
  <? 
  while1("*","epzhu_orderlogkf where orderbh='".$orderbh."' and selluserid=".$userid." order by sj asc");while($row1=mysql_fetch_array($res1)){
  $txt=$row1[txt];
 ...
 f1.action="orderjf2kf.php?orderbh=<?=$orderbh?>";
 }
 </script>
