...
while0("*","epzhu_orderwz where orderbh='".$orderbh."' and (ddzt='db' or ddzt='backerr') and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderwz.php");}


if(sqlzhuru($_POST[jvs])=="sh"){
 zwzr();
 $zfmm=sha1(sqlzhuru($_POST[t1]));
 if(panduan("uid,zfmm","epzhu_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("支付密码有误！","shouhuowz.php?orderbh=".$orderbh);}
 ...
 //推荐E
 $sj=date("Y-m-d H:i:s");
 updatetable("epzhu_orderwz","ddzt='suc',oksj='".$sj."' where userid=".$userid." and id=".$row[id]);
...
<div class="right">
 ...
 f1.action="shouhuowz.php?orderbh=<?=$orderbh?>";
 }
 </script>
