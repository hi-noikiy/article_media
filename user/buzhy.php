<?
 ...
 if($carid[$i]!=""){
  $sqlc="select * from epzhu_carzh where id=".$carid[$i];mysql_query("SET NAMES 'GBK'");$resc=mysql_query($sqlc);if($rowc=mysql_fetch_array($resc)){
  $sql="select * from epzhu_prozh where bh='".$rowc[probh]."' and zt=0 and ifxj=0";mysql_query("SET NAMES 'GBK'");$res=mysql_query($sql);
  ...
  intotable("epzhu_orderzh","probh,sj,uip,selluserid,userid,money1,orderbh,num,tit,ddzt,tcv,buyform,tcid,fhxs,shdz,yunfei,liuyan","'".$row[bh]."','".$sj."','".$uip."',".$row[userid].",".$rowc[userid].",".$rowc[money1].",'".$orderbh."',".$rowc[num].",'".$row[tit]."','wait','".$rowc[tcv]."','".$rowc[buyform]."',".$rowc[tcid].",".$row[fhxs].",'".$rowc[shdz]."',".$rowc[yunfei].",'".$rowc[bz]."'");
  PointUpdateM($rowc[userid],$allmoney*(-1));
  PointIntoM($rowc[userid],"购买商品,数量".$rowc[num],$allmoney*(-1));
  updatetable("epzhu_prozh","xsnum=xsnum+".$rowc[num]." where id=".$row[id]);
  
  if(empty($rowc[tcid]) || empty($rowc[tcfhxs])){
  $kc=$row[kcnum]-$rowc[num];updatetable("epzhu_prozh","kcnum=".$kc." where id=".$row[id]);
  //无套餐或套餐跟随商品自动发货商品B
  if($row[fhxs]==2 || $row[fhxs]==3 || $row[fhxs]==4){
  updatetable("epzhu_typezh","fhsj='".$sj."',ddzt='db' where ddzt='wait' and orderbh='".$orderbh."'");
  $dbsj=$rowcontrol[dbsj];
  $sqldb="select * from epzhu_typezh where id=".$row[ty1id];mysql_query("SET NAMES 'GBK'");$resdb=mysql_query($sqldb);if($rowdb=mysql_fetch_array($resdb)){
  ...
  updatetable("epzhu_typezh","txt='".$txt."' where orderbh='".$orderbh."'");
  ...
  //写入评价
  $sql2="INSERT INTO `epzhu_propjzh` SET `probh`='".$rowc[probh]."',`selluserid`=".$rowm[id].",`userid`=".$rowu[id].",`sj`='".date('Y-m-d H:i:s',time())."',`uip`='127.0.0.1（本机）',`pf1`=5,`pf2`=5,`pf3`=5,`txt`='暂未评价',`orderbh`='".$orderbh."',`pjlx`=1,`iftp`=0;";
  mysql_query($sql2); 
  //写入邮件E
  ...
  deletetable("epzhu_carzh where id=".$rowc[id]);
  //////////////////////////////////结束逐一购买
  }
 }	 
 }
