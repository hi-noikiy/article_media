...
while0("*","epzhu_orderkf where orderbh='".$orderbh."' and (ddzt='db' or ddzt='backerr') and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderkf.php");}


if(sqlzhuru($_POST[jvs])=="sh"){
 zwzr();
 $zfmm=sha1(sqlzhuru($_POST[t1]));
 if(panduan("uid,zfmm","epzhu_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("支付密码有误！","shouhuokf.php?orderbh=".$orderbh);}
 ...
 //推荐E
 $sj=date("Y-m-d H:i:s");
 updatetable("epzhu_orderkf","ddzt='suc',oksj='".$sj."' where userid=".$userid." and id=".$row[id]);
...
<div class="right">
 ...
 f1.action="shouhuokf.php?orderbh=<?=$orderbh?>";
 }
 </script>
