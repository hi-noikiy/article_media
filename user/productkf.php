...
if(empty($anzhuang)){
 ...
 if($money1<0 || $money2<0){Audit_alert("价格不能为负数！","productlistkf.php");}
 $fhxs=intval(sqlzhuru($_POST[Rfhxs]));
 if($rowcontrol[ifproduct]=="on"){$nzt=0;}else{$nzt=1;}
 if(!empty($anzhuang) && !is_numeric($anzhuang)){Audit_alert("安装费请输入数字","productlistkf.php");}
 ...
 if(1==$ifuserjd){
 deletetable("epzhu_prouserdjkf where probh='".$bh."'");
  for($i=1;$i<intval($_POST[djnum]);$i++){
  $zhekou=$_POST["zhekou_".$i];
  $djname=$_POST["name1_".$i];
  if(!empty($zhekou)){intotable("epzhu_prouserdjkf","probh,userid,djname,admin,zhi","'".$bh."',".$row[userid].",'".$djname."',1,".$zhekou."");}
  }
 }

 updatetable("epzhu_prokf",$ses."
 ...
 if(3==$fhxs){
  ...
  delFile($lj.$row[upf]);
  updatetable("epzhu_prokf","upf='".$mc."' where bh='".$bh."' and userid=".$userid);
  }
 }
 //上传E
 //卡密B
 ...
 php_toheader("prosuckf.php?bh=".$bh."&id=".$row[id]);
...
tjwait();
f1.action="productkf.php?bh=<?=$bh?>&control=update";
}
function bzonc(x){
...
location.href="protypelistkf.php";
}
...
<div class="right">
 ...
 [<a href="productlxkf.php?action=update&id=<?=$row[id]?>">修改</a>]
 </li>
 <li class="l1">自定义分组：</li>
 <li class="l2">
 <select name="mty" class="inp1">
 <option value="0yjcode0">选择分组</option>
 <? while1("*","epzhu_protypekf where admin=1 and zt=0 and userid=".$luserid);while($row1=mysql_fetch_array($res1)){?>
 ...
 <? while2("*","epzhu_protypekf where admin=2 and name1='".$row1[name1]."' and zt=0 and userid=".$luserid);while($row2=mysql_fetch_array($res2)){?>
 ...
 <? $i=1;while1("*","epzhu_typesxkf where admin=1 and typeid=".$row[ty1id]." order by xh asc");while($row1=mysql_fetch_array($res1)){?>
 <input type="hidden" value="<?=$row1[id]?>" name="tysxty1_<?=$i?>" />
 <ul class="uk1">
 <li class="l1"><?=$row1[name1]?>：</li>
 <li class="l2">
 <? $j=1;while2("*","epzhu_typesxkf where admin=2 and name1='".$row1[name1]."' and typeid=".$row1[typeid]." order by xh asc");while($row2=mysql_fetch_array($res2)){?>
...
<div class="layui-form-item">
 ...
 while2("*","epzhu_prouserdjkf where probh='".$bh."' and djname='".$row1[name1]."'");if($row2=mysql_fetch_array($res2)){$zhekou=$row2[zhi];}else{$zhekou=$row1[zhekou];}
 ?>
 <ul class="dju2">
 ...
 <ul class="uk">
 <li class="l3"></li>
 <li class="l4"><? tjbtnr("提交","productlistkf.php")?></li>
 </ul>
 </form>
 <!--E-->
</div> 
