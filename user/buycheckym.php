<?
$carid=$_GET[carid];
if($carid==""){php_toheader("carym.php");}
...
for($i=0;$i<count($c);$i++){
 ...
 $num=intval($d[1]);
 if($num<=0){Audit_alert("错误，购买数量不得少于1，返回重试","carym.php");}
 
 while0("*","epzhu_carym where userid=".$rowuser[id]." and id=".$id."");if(!$row=mysql_fetch_array($res)){php_toheader("carym.php");}
 
 while1("*","epzhu_proym where bh='".$row[probh]."' and zt=0 and ifxj=0");if(!$row1=mysql_fetch_array($res1)){Audit_alert("商品已下架或未审核","carym.php");}
 ...
 if(empty($row[tcid])){$zm=$money;$tcv="";$tcfhxs=99;}
  ...
  while2("*","epzhu_taocan where id=".$row[tcid]);if(!$row2=mysql_fetch_array($res2)){Audit_alert("套餐已下架，请联系客服","carym.php");}
  $zm=$row2[money1];
  $tcfhxs=$row2[fhxs];
 ...
 if($kcnum<$num){Audit_alert("库存不够了","carym.php");}
 ...
 if(!empty($row1[ifuserdj])){
  ...
  $sqlu4="select * from epzhu_prouserdjym where probh='".$row[probh]."' and djname='".$djname."'";mysql_query("SET NAMES 'GBK'");$resu4=mysql_query($sqlu4);
 ...
 updatetable("epzhu_carym","num=".$num.",money1=".$zm.",tcv='".$tcv."',shdz='".$shdz."',yunfei=".$yf.",tcfhxs=".$tcfhxs." where id=".$id);
