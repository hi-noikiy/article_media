...
if($_POST[jvs]=="smrz"){
 zwzr();
 if(panduan("uid,sfzrz","epzhu_user where uid='".$_SESSION[SHOPUSER]."' and (sfzrz=2 or sfzrz=3)")==0){Audit_alert("错误的请求！","smrz.php");}
 $sfz=sqlzhuru($_POST[tsfz]);
 if(strlen(stripos($sfz,"/"))>0 || strlen(stripos($sfz,";"))>0){Audit_alert("身份证号码格式有误！","smrz.php");}
 updatetable("epzhu_user","uname='".sqlzhuru($_POST[tuname])."',sfz='".sqlzhuru($_POST[tsfz])."',sfzrz=0 where uid='".$_SESSION[SHOPUSER]."'");
