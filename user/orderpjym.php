...
while0("*","epzhu_orderym where orderbh='".$orderbh."' and ddzt='suc' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderym.php");}
if(panduan("bh","epzhu_proym where bh='".$row[probh]."'")==0){Audit_alert("该商品已被删除，无法进行评价！","orderym.php");}
$sqlpj="select * from epzhu_propjym where orderbh='".$orderbh."' and userid=".$userid;mysql_query("SET NAMES 'GBK'");$respj=mysql_query($sqlpj);
if($rowpj=mysql_fetch_array($respj)){$ifpj=1;}else{$ifpj=0;}

if(sqlzhuru($_POST[jvs])=="pj"){
 zwzr();
 if(1==$ifpj){Audit_alert("您已评价过，不能重复点评！","orderpjym.php?orderbh=".$orderbh);}
 $sj=date("Y-m-d H:i:s");
 $uip=$_SERVER["REMOTE_ADDR"];
 if(panduan("*","epzhu_tp where bh='".$orderbh."'")==1){$iftp=1;}else{$iftp=0;}
 intotable("epzhu_propjym","probh,selluserid,userid,sj,uip,pf1,pf2,pf3,txt,orderbh,pjlx,iftp","'".$row[probh]."',".$row[selluserid].",".$row[userid].",'".$sj."','".$uip."',".sqlzhuru($_POST[hpjjf1]).",".sqlzhuru($_POST[hpjjf2]).",".sqlzhuru($_POST[hpjjf3]).",'".sqlzhuru($_POST[s1])."','".$row[orderbh]."',".$_POST[Rpj].",".$iftp."");$id=mysql_insert_id();
 if(empty($id)){Audit_alert("您已评价过，不能重复点评！","orderpjym.php?orderbh=".$orderbh);}
 $sql1="select avg(pf1) as pf1v,avg(pf2) as pf2v,avg(pf3) as pf3v from epzhu_propjym where probh='".$row[probh]."' and selluserid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$res1=mysql_query($sql1);
 $row1=mysql_fetch_array($res1);
 updatetable("epzhu_proym","pf1=".round($row1[pf1v],2).",pf2=".round($row1[pf2v],2).",pf3=".round($row1[pf3v],2)." where bh='".$row[probh]."'");
 PointInto($userid,"交易成功，点评商品获得积分",$rowcontrol[pjjf]);
 PointUpdate($userid,$rowcontrol[pjjf]); 

 $sqlp="select avg(pf1) pf1v,avg(pf2) pf2v,avg(pf3) pf3v from epzhu_proym where zt=0 and userid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");
 ...
 php_toheader("orderpjym.php?orderbh=".$orderbh);
...
if(sqlzhuru($_POST[jvs])=="xgpj"){
 ...
 updatetable("epzhu_propjym","pf1=".sqlzhuru($_POST[hpjjf1]).",pf2=".sqlzhuru($_POST[hpjjf2]).",pf3=".sqlzhuru($_POST[hpjjf3]).",txt='".sqlzhuru($_POST[s1])."',pjlx=".$_POST[Rpj].",iftp=".$iftp." where orderbh=".$orderbh);
 $sql1="select avg(pf1) as pf1v,avg(pf2) as pf2v,avg(pf3) as pf3v from epzhu_propjym where probh='".$row[probh]."' and selluserid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$res1=mysql_query($sql1);
 $row1=mysql_fetch_array($res1);
 updatetable("epzhu_proym","pf1=".round($row1[pf1v],2).",pf2=".round($row1[pf2v],2).",pf3=".round($row1[pf3v],2)." where bh='".$row[probh]."'");
 PointInto($userid,"交易成功，点评商品获得积分",$rowcontrol[pjjf]);
...
function tj(){
 ...
 tjwait();
 f1.action="orderpjym.php?orderbh=<?=$orderbh?>";
}
</script>
</head>
<body>
