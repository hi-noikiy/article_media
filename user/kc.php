...
if($_GET[control]=="add"){
 ...
 for($i=0;$i<=count($d);$i++){
  ...
  if(panduan("probh,userid,ka","epzhu_kc where probh='".$bh."' and ka='".sqlzhuru($_POST[tka])."' and userid=".$userid)==1){
  Audit_alert("卡号已存在，添加失败!","kc.php?bh=".$bh);
  }
  intotable("epzhu_kc","probh,userid,ka,mi,ifok","'".$bh."',".$userid.",'".sqlzhuru($_POST[tka])."','".sqlzhuru($_POST[tmi])."',0");
  ...
  $mi= $data->sheets[0]['cells'][$i][2]."";
   if(panduan("probh,userid,ka","epzhu_kc where probh='".$bh."' and ka='".$ka."' and userid=".$userid)==0){
   intotable("epzhu_kc","probh,userid,ka,mi,ifok","'".$bh."',".$userid.",'".$ka."','".$mi."',0");
   }
  }
  //导入结束
  delFile($mu.$bh.".xls");
 ...
 zwzr();
 $id=$_GET[id];
 if(panduan("id,probh,userid,ka","epzhu_kc where probh='".$bh."' and ka='".sqlzhuru($_POST[tka])."' and id<>".$id." and userid=".$userid)==1){
...
<div class="right">
 ...
 while0("*","epzhu_kc where userid=".$luserid." and id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("kclist.php?bh=".$bh);}
 ?>
 <script language="javascript">
 function tj(){
  tjwait();
