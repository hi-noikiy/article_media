...
$sj=date("Y-m-d H:i:s");
include("buycheckym.php");

//B
if(sqlzhuru($_POST[jvs])=="carpay" && sqlzhuru($_POST[R1])=="alipay"){
if($needmoney<=$usermoney){Audit_alert("您的可用余额充足，请用余额直接支付。","carpayym.php?carid=".$carid);}
zwzr();
$bh=time();
...
$return_url = weburl."user/orderym.php";//页面跳转同步通知页面路径
...
echo $html_text;exit;
 ...
 if($needmoney>$usermoney){Audit_alert("您的可用余额不足，返回重试。","carpayym.php?carid=".$carid);}
 zwzr();
 include("buyym.php");
 php_toheader("orderym.php");
...
function carpaytj(x){
 ...
 tjwait();
 if(rv=="alipay" || rv==""){fu="carpayym.php?carid="+x;}
 else if(rv=="tenpay"){fu="tenpay/buy_indexym.php?carid="+x;}
...
<div class="right">
 ...
 <div class="carpay">
  ...
  <li class="l3">
  <a href="orderym.php">完成付款</a>
  <a href="javascript:void(0);" onClick="javascript:alert('付款遇到问题？请联系客服');location.href='carpayym.php?carid=<?=$carid?>';return false;">付款遇到问题</a>
  </li>
  </ul>
  </div>
