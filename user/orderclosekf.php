...
if(sqlzhuru($_POST[jvs])=="close"){
 ...
 if(panduan("uid,zfmm","epzhu_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("支付密码有误！","orderclosekf.php?orderbh=".$orderbh);}
 if($row[ddzt]!="wait"){Audit_alert("未知错误！","orderviewkf.php?orderbh=".$orderbh);}
 $allmoney=$row[money1]*$row[num]+$row[yunfei];
 PointUpdateM($userid,$allmoney);
 PointIntoM($userid,"取消订单",$allmoney);
 $sj=date("Y-m-d H:i:s");
 updatetable("epzhu_orderkf","ddzt='close',closesj='".$sj."' where userid=".$userid." and id=".$row[id]);
...
<div class="right">
 ...
 f1.action="orderclosekf.php?orderbh=<?=$orderbh?>";
 }
 </script>
