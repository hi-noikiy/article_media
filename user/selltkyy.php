...
while0("*","epzhu_orderyy where orderbh='".$orderbh."' and selluserid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderyy.php");}


if(sqlzhuru($_POST[jvs])=="tk"){
 zwzr();
 $zfmm=sha1(sqlzhuru($_POST[t1]));
 if(panduan("uid,zfmm","epzhu_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("支付密码有误！","selltkyy.php?orderbh=".$orderbh);}
 if($row[ddzt]!="back"){Audit_alert("未知错误！","sellorderviewyy.php?orderbh=".$orderbh);}
 ...
 if(sqlzhuru($_POST[R1])=="yes"){
  ...
  updatetable("epzhu_orderyy","ddzt='backsuc',tksj='".$row1[sj]."',tkly='".$row1[tkly]."',tkjg='".sqlzhuru($_POST[t2])."',tkoksj='".$sj."' where selluserid=".$userid." and id=".$row[id]);
 ...
 //不同意B
  ...
  updatetable("epzhu_orderyy","ddzt='backerr',tksj='".$row1[sj]."',tkly='".$row1[tkly]."',tkjg='".sqlzhuru($_POST[t2])."',tkoksj='".$sj."' where selluserid=".$userid." and id=".$row[id]);
  deletetable("epzhu_tkkf where orderbh='".$orderbh."' and selluserid=".$userid);
 }
 //不同意E
...
<div class="right">
 ...
 <li class="l1"><a href="sellorderyy.php">所有订单</a></li>
 </ul>
 ...
 tjwait();
 f1.action="selltkyy.php?orderbh=<?=$orderbh?>";
 }
 </script>
