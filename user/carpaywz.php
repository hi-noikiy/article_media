...
$sj=date("Y-m-d H:i:s");
include("buycheckwz.php");

//B
if(sqlzhuru($_POST[jvs])=="carpay" && sqlzhuru($_POST[R1])=="alipay"){
if($needmoney<=$usermoney){Audit_alert("您的可用余额充足，请用余额直接支付。","carpaywz.php?carid=".$carid);}
zwzr();
$bh=time();
...
$return_url = weburl."user/orderwz.php";//页面跳转同步通知页面路径
...
echo $html_text;exit;
 ...
 if($needmoney>$usermoney){Audit_alert("您的可用余额不足，返回重试。","carpaywz.php?carid=".$carid);}
 zwzr();
 include("buywz.php");
 php_toheader("orderwz.php");
...
function carpaytj(x){
 ...
 tjwait();
 if(rv=="alipay" || rv==""){fu="carpaywz.php?carid="+x;}
 else if(rv=="tenpay"){fu="tenpay/buy_indexwz.php?carid="+x;}
...
<div class="right">
 ...
 <div class="carpay">
  ...
  <li class="l3">
  <a href="orderwz.php">完成付款</a>
  <a href="javascript:void(0);" onClick="javascript:alert('付款遇到问题？请联系客服');location.href='carpaywz.php?carid=<?=$carid?>';return false;">付款遇到问题</a>
  </li>
  </ul>
  </div>
