...
<div class="right">
 ...
 <ul class="ksedi">
 <li class="l2">
 <a href="javascript:NcheckDEL(3,'epzhu_task')" class="a1">删除</a>
 ...
 while1("*","epzhu_taskhf where bh='".$row[bh]."' order by money1 desc");$row1=mysql_fetch_array($res1);$moneyg=$row1[money1];$moneyg=returnjgdw($moneyg,"",0);
 while1("*","epzhu_taskhf where bh='".$row[bh]."' order by money1 asc");$row1=mysql_fetch_array($res1);$moneyd=$row1[money1];$moneyd=returnjgdw($moneyd,"",0);
 $bmnum=returncount("epzhu_taskhf where bh='".$row[bh]."' and userid=".$luserid);
 taskok($row["id"]);
 ?>
 <ul class="tasklist">
