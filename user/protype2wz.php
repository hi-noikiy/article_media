...
$bh=$_GET[bh];
while0("*","epzhu_protypewz where userid=".$userid." and admin=2 and bh='".$bh."'");
if(!$row=mysql_fetch_array($res)){Audit_alert("来源错误","protypelistwz.php","parent.");}

if($_GET[control]=="update"){
 zwzr();
 if(panduan("*","epzhu_protypewz where admin=2 and name1='".sqlzhuru($_POST[tname1])."' and name2='".sqlzhuru($_POST[tname2])."' and id<>".$row[id])==1)
 {Audit_alert("该分组已存在！","protype2wz.php?bh=".$bh);}
 updatetable("epzhu_protypewz","name2='".sqlzhuru($_POST[tname2])."',xh=".sqlzhuru($_POST[txh]).",zt=0 where id=".$row[id]);
 php_toheader("protype2wz.php?t=suc&bh=".$bh);
}
?>
...
f1.action="protype2wz.php?control=update&bh=<?=$bh?>";
}
<? if($_GET["t"]=="suc"){?>
