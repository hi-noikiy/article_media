...
$bh=$_GET[bh];
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_adlx where bh='".$bh."' and fflx=1 and admin=1");if(!$row=mysql_fetch_array($res)){php_toheader("adlx1.php");}
if(empty($row[maxnum])){$sywz="充足";}
else{
 $asy=$row[maxnum]-returncount("epzhu_ad where adbh='".$row[adbh]."' and zt=0");
...
if($_GET[control]=="add"){  //表示新增
 ...
 $nxh=returnxh("epzhu_ad"," and adbh='".$row[adbh]."'");
...
<div class="right">
 ...
 <li class="l1">排队人数：</li>
 <li class="l21"><?=returncount("epzhu_ad where adbh='".$row[adbh]."' and zt=1")?></li>
 <li class="l1">广告尺寸：</li>
 ...
 <? while1("*","epzhu_adlx where admin=2 and bh='".$bh."' order by id asc");while($row1=mysql_fetch_array($res1)){?>
