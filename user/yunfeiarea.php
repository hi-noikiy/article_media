...
while0("*","epzhu_yunfei where id=".$id." and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("yunfeilist.php");}

if($_GET[control]=="update"){
 $area="|".sqlzhuru($_POST[area1]).",".sqlzhuru($_POST[add2]).",".sqlzhuru($_POST[add3])."|";
 if(strstr($row[cityid],$area)==""){$areav=$row[cityid].$area;updatetable("epzhu_yunfei","cityid='".$areav."' where id=".$id." and userid=".$userid);}
 ...
 updatetable("epzhu_yunfei","cityid='".$str."' where id=".$id." and userid=".$userid);
