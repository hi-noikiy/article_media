...
if(sqlzhuru($_POST[jvs])=="fh"){
 ...
 updatetable("epzhu_orderwz","fhsj='".$sj."',ddzt='db',kdid=".$kdid.",kddh='".sqlzhuru($_POST[tkddh])."' where ddzt='wait' and orderbh='".$orderbh."' and selluserid=".$userid);
 while1("bh,ty1id","epzhu_prowz where bh='".$row[probh]."'");if($row1=mysql_fetch_array($res1)){$tyid=$row1[ty1id];}else{$tyid=0;}
 $dbsj=$rowcontrol[dbsj];
 $sqldb="select * from epzhu_typewz where id=".$tyid;mysql_query("SET NAMES 'GBK'");$resdb=mysql_query($sqldb);if($rowdb=mysql_fetch_array($resdb)){
...
<div class="right">
 ...
 tjwait();
 f1.action="fahuowz.php?orderbh=<?=$orderbh?>";
 }
 </script>
