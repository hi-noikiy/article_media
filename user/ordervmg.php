<? 
 $sj=date("Y-m-d H:i:s");
 $au="../productmg/view".returnproid($row[probh]).".html";
 ...
 <div class="orderv1">
  ...
  <div class="ztcontrol">
   ...
   <div class="d2">
    ...
    <div class="ds2"><strong>提醒：</strong>如果卖家长时间未发货，您可以【<a href="orderclosemg.php?orderbh=<?=$orderbh?>">取消订单</a>】。卖家也挺不容易，您可以【<a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$rows1[uqq]?>&site=<?=weburl?>&menu=yes" class="s2" target="_blank">提醒卖家尽快发货</a>】。</div>
    <div class="ds3">
    <a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$rows1[uqq]?>&site=<?=weburl?>&menu=yes" class="a1" target="_blank">提醒卖家尽快发货</a>
    <a href="orderclosemg.php?orderbh=<?=$orderbh?>" class="a0">取消订单</a>
    </div>
    <? }?>
    ...
    <div class="ds2"><strong>提醒：</strong>如果还需要该商品，请点<a href="<?=$au?>" target="_blank">再次购买</a>。</div>
    <? if(panduan("orderbh,userid","epzhu_propjmg where orderbh='".$orderbh."' and userid=".$userid)==0){?>
    <div class="ds3"><a href="orderpjmg.php?orderbh=<?=$orderbh?>" class="a1">写评价赚积分</a></div>
    <? }?>
    <? }?>
    ...
    <div class="ds2"><strong>提醒：</strong>如果商品没有问题，你可以进行【<a href="shouhuomg.php?orderbh=<?=$orderbh?>">确认收货</a>】，如果商品有问题，你可以【<a href="../help/aboutview4.html" target="_blank">申请客服介入</a>】<br>请务必在 <span class="red"><?=$sjcv?></span> 内进行相关操作，否则系统自动完成该笔订单，资金会打入卖家账户</div>
    <div class="ds3">
    <a href="shouhuomg.php?orderbh=<?=$orderbh?>" class="a1">确认收货</a>
    <a href="orderjfmg.php?orderbh=<?=$orderbh?>" class="a0">申请客服介入</a>
    </div>
    <? }?>
    ...
    <div class="ds2"><strong>提醒：</strong>如果商品本身及卖家方面都无问题，你也还需要该商品，那么你可以<a href="orderqxtkmg.php?orderbh=<?=$orderbh?>">取消退款申请</a>。<br>你在 <?=$row[tksj]?> 申请了退款<br>你的退款理由：<?=$row[tkly]?></div>
    <div class="ds3">
    <a href="orderqxtkmg.php?orderbh=<?=$orderbh?>" class="a1">取消退款申请</a>
    </div>
    <? }?>
    ...
    <div class="ds2"><strong>提醒：</strong>当您收到商品且确认没问题后，可进行【<a href="shouhuomg.php?orderbh=<?=$orderbh?>">确认收货</a>】来完成此次交易;在此期间请尽量注意资金担保剩余时间，若临近该时间点，商品依然存在问题且卖方未予及时解决，可先【<a href="orderycmg.php?orderbh=<?=$orderbh?>">延长担保时间</a>】或【<a href="ordertkmg.php?orderbh=<?=$orderbh?>">申请退款</a>】，等待卖方解决问题后，再【<a href="shouhuomg.php?orderbh=<?=$orderbh?>">确认收货</a>】或【<a href="orderqxtkmg.php?orderbh=<?=$orderbh?>">取消退款</a>】，不可轻信卖方许诺，以防有意拖延时间至自动完成交易。资金担保时间结束后，款项将自动转入卖家账户。</div>
    <div class="ds3">
    <a href="shouhuomg.php?orderbh=<?=$orderbh?>" class="a1">确认收货</a>
    <a href="orderycmg.php?orderbh=<?=$orderbh?>" class="a0">延长担保时间</a>
    <a href="ordertkmg.php?orderbh=<?=$orderbh?>" class="a0">申请退款</a>
    <a href="orderqxtkmg.php?orderbh=<?=$orderbh?>" class="a0">取消退款</a>
    </div>
    <? }?>

    <? if($row[ddzt]=="jf"){?>
    <div class="ds1">您已经申请了平台客服介入处理，处理过程中资金将被冻结，直至处理完毕。您可以提交更有利于您的证据。</div>
    <div class="ds3"><a href="orderjf1mg.php?orderbh=<?=$orderbh?>" class="a1">提交新证据</a></div>
    <? }?>

    <? if($row[ddzt]=="jfbuy"){?>
    <div class="ds1">平台已经判定本次交易纠纷为买家胜诉，款项已经退回到您的账户。</div>
    <div class="ds3"><a href="orderjf1mg.php?orderbh=<?=$orderbh?>" class="a0">查看沟通记录</a></div>
    <? }?>
  ...
  <td class="td2">
  <? 
  while1("*","epzhu_promg where bh='".$row[probh]."'");if($row1=mysql_fetch_array($res1)){
  $tcfhxs=0;
  if(!empty($row[tcid])){
