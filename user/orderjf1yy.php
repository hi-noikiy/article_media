...
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_orderyy where orderbh='".$orderbh."' and (ddzt='jf' or ddzt='jfbuy' or ddzt='jfsell') and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderyy.php");}

if(sqlzhuru($_POST[jvs])=="jf"){
 zwzr();
 $txt=sqlzhuru1($_POST[content]);
 if(empty($txt)){Audit_alert("申请理由内容不得为空，返回重试！","orderjf1yy.php?orderbh=".$orderbh);}
 intotable("epzhu_orderyylogkf","orderbh,userid,selluserid,admin,txt,sj","'".$orderbh."',".$row[userid].",".$row[selluserid].",1,'".$txt."','".$sj."'");
 php_toheader("orderjf1yy.php?orderbh=".$orderbh); 
...
<div class="right">
 ...
 <div class="jfgtlist">
  ...
  <? 
  while1("*","epzhu_orderyylogkf where orderbh='".$orderbh."' and userid=".$userid." order by sj asc");while($row1=mysql_fetch_array($res1)){
  $txt=$row1[txt];
 ...
 f1.action="orderjf1yy.php?orderbh=<?=$orderbh?>";
 }
 </script>
