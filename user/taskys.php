...
$sqltask="select * from epzhu_task where bh='".$bh."' and taskty=0 and zt=4 and userid=".$userid."";mysql_query("SET NAMES 'GBK'");$restask=mysql_query($sqltask);
if(!$rowtask=mysql_fetch_array($restask)){php_toheader("tasklist.php");}

$sqltaskhf="select * from epzhu_taskhf where bh='".$bh."' and taskty=0 and useridhf=".$rowtask[useridhf]." and ifxz=1";mysql_query("SET NAMES 'GBK'");$restaskhf=mysql_query($sqltaskhf);
...
if($_GET[control]=="ys"){
 ...
 if($zt=="yes"){
  ...
  updatetable("epzhu_task","zt=5 where id=".$rowtask[id]);
  $txt="验收通过";
  intotable("epzhu_tasklog","bh,userid,useridhf,admin,txt,sj,fj","'".$bh."',".$rowtask[userid].",".$rowtask[useridhf].",1,'".$txt."','".$sj."',''");
 }elseif($zt=="no"){
  $txt="验收不通过，申请平台介入";
  intotable("epzhu_tasklog","bh,userid,useridhf,admin,txt,sj,fj","'".$bh."',".$rowtask[userid].",".$rowtask[useridhf].",1,'".$txt."','".$sj."',''");
  updatetable("epzhu_task","zt=8 where id=".$rowtask[id]);
 }
