...
$ty1id=$_GET[ty1id];
while0("*","epzhu_taocan where userid=".$userid." and id=".$ty1id);$row=mysql_fetch_array($res);
$ty1tit=$row[tit];
...
if($_GET[control]=="update"){
 zwzr();
 if(panduan("*","epzhu_taocan where userid=".$userid." and admin=2 and tit='".sqlzhuru($_POST[t0])."' and tit2='".sqlzhuru($_POST[t1])."' and probh='".$bh."' and id<>".$id)==1){Audit_alert("该套餐说明已存在！","taocan1.php?action=update&id=".$id."&ty1id=".$_GET[ty1id]."&bh=".$bh);}
 
 $kcnum=sqlzhuru($_POST[tkcnum]);if(!is_numeric($kcnum)){$kcnum=0;}
 $fhxs=intval(sqlzhuru($_POST[Rfhxs]));
 updatetable("epzhu_taocan","tit2='".sqlzhuru($_POST[t1])."',
 ...
 uploadtpnodata(2,"upload/".$row[userid]."/".$row[probh]."/","tc".$id.".png","allpic",34,34,0,0,"no");
 //上传B
 if(3==$fhxs){
  ...
  delFile($lj.$row[upf]);
  updatetable("epzhu_taocan","upf='".$mc."' where id=".$id." and userid=".$userid);
  }
 }
 //上传E
 //卡密B
 ...
 for($i=0;$i<=count($d);$i++){
  if(!empty($d[$i])){
   $e=preg_split("/\s/",$d[$i]);
     if(panduan("probh,tcid,userid,ka","epzhu_taocan_kc where probh='".$bh."' and ka='".$ka."' and tcid=".$id." and userid=".$userid)==0){
     $mi="";
	 if(count($e)>=2){for($ei=1;$ei<count($e);$ei++){$mi=$mi." ".$e[$ei];}}
	 intotable("epzhu_taocan_kc","probh,tcid,userid,ka,mi,ifok","'".$bh."',".$id.",".$userid.",'".$e[0]."','".$mi."',0");
	 }
  }
 }
 kamikc_tc($bh,$id);
...
<div class="right">
 ...
 <li class="l1">套餐图标：</li>
 <li class="l2"><input type="file" name="inp2" id="inp2" size="15" accept=".jpg,.gif,.jpeg,.png"> 34*34,不上传则显示文字形式</li>
