...
if(panduan("bh","epzhu_pro where bh='".$row[probh]."'")==0){Audit_alert("该商品已被删除，无法进行评价！","order.php");}
...
if(sqlzhuru($_POST[jvs])=="pj"){
 ...
 intotable("epzhu_propj","probh,selluserid,userid,sj,uip,pf1,pf2,pf3,txt,orderbh,pjlx,iftp","'".$row[probh]."',".$row[selluserid].",".$row[userid].",'".$sj."','".$uip."',".sqlzhuru($_POST[hpjjf1]).",".sqlzhuru($_POST[hpjjf2]).",".sqlzhuru($_POST[hpjjf3]).",'".sqlzhuru($_POST[s1])."','".$row[orderbh]."',".$_POST[Rpj].",".$iftp."");$id=mysql_insert_id();
...
if(sqlzhuru($_POST[jvs])=="xgpj"){
 ...
 updatetable("epzhu_propj","pf1=".sqlzhuru($_POST[hpjjf1]).",pf2=".sqlzhuru($_POST[hpjjf2]).",pf3=".sqlzhuru($_POST[hpjjf3]).",txt='".sqlzhuru($_POST[s1])."',pjlx=".$_POST[Rpj].",iftp=".$iftp." where orderbh=".$orderbh);
 $sql1="select avg(pf1) as pf1v,avg(pf2) as pf2v,avg(pf3) as pf3v from epzhu_propj where probh='".$row[probh]."' and selluserid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$res1=mysql_query($sql1);
 $row1=mysql_fetch_array($res1);
 updatetable("epzhu_pro","pf1=".round($row1[pf1v],2).",pf2=".round($row1[pf2v],2).",pf3=".round($row1[pf3v],2)." where bh='".$row[probh]."'");
 PointInto($userid,"交易成功，点评商品获得积分",$rowcontrol[pjjf]);
