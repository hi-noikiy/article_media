...
if($_POST[jvs]=="openshop"){
 ...
 if(panduan("uid,pwd","epzhu_user where uid='".$_SESSION[SHOPUSER]."' and pwd='".$t1."'")==0){Audit_alert("登录密码验证失败，返回重试！","openshop4.php");}
 $d=preg_split("/xcf/",$_POST[d1]);
 while1("*","epzhu_openyue where id=".$d[2]);if(!$row1=mysql_fetch_array($res1)){Audit_alert("操作失败，返回重试！","openshop4.php");}
 $m=$row1[money1];
 ...
 updatetable("epzhu_user","shopzt=2,dqsj='".$dqsj."' where uid='".$_SESSION[SHOPUSER]."'");
...
<div class="right">
 <ul class="wz">
 <li class="l0">请选择：</li>
 <li class="g_ac0_h g_bc0_h"><a href="openshop4.php">店铺续费</a></li>
 </ul>
 ...
 <li class="l1">续费期限：</li>
 <li class="l2 red">
 <select name="d1" class="inp1" onchange="fycha()">
 <? while1("*","epzhu_openyue order by yue asc");while($row1=mysql_fetch_array($res1)){if($row1[yue] % 12==0){$nd=$row1[yue]/12;$nd=$nd."年";}else{$nd=$row1[yue]."个月";}?>
