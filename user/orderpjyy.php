...
while0("*","epzhu_orderyy where orderbh='".$orderbh."' and ddzt='suc' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderyy.php");}
if(panduan("bh","epzhu_proyy where bh='".$row[probh]."'")==0){Audit_alert("该商品已被删除，无法进行评价！","orderyy.php");}
$sqlpj="select * from epzhu_propjyy where orderbh='".$orderbh."' and userid=".$userid;mysql_query("SET NAMES 'GBK'");$respj=mysql_query($sqlpj);
if($rowpj=mysql_fetch_array($respj)){$ifpj=1;}else{$ifpj=0;}

if(sqlzhuru($_POST[jvs])=="pj"){
 zwzr();
 if(1==$ifpj){Audit_alert("您已评价过，不能重复点评！","orderpjyy.php?orderbh=".$orderbh);}
 $sj=date("Y-m-d H:i:s");
 $uip=$_SERVER["REMOTE_ADDR"];
 if(panduan("*","epzhu_tp where bh='".$orderbh."'")==1){$iftp=1;}else{$iftp=0;}
 intotable("epzhu_propjyy","probh,selluserid,userid,sj,uip,pf1,pf2,pf3,txt,orderbh,pjlx,iftp","'".$row[probh]."',".$row[selluserid].",".$row[userid].",'".$sj."','".$uip."',".sqlzhuru($_POST[hpjjf1]).",".sqlzhuru($_POST[hpjjf2]).",".sqlzhuru($_POST[hpjjf3]).",'".sqlzhuru($_POST[s1])."','".$row[orderbh]."',".$_POST[Rpj].",".$iftp."");$id=mysql_insert_id();
 if(empty($id)){Audit_alert("您已评价过，不能重复点评！","orderpjyy.php?orderbh=".$orderbh);}
 $sql1="select avg(pf1) as pf1v,avg(pf2) as pf2v,avg(pf3) as pf3v from epzhu_propjyy where probh='".$row[probh]."' and selluserid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$res1=mysql_query($sql1);
 $row1=mysql_fetch_array($res1);
 updatetable("epzhu_proyy","pf1=".round($row1[pf1v],2).",pf2=".round($row1[pf2v],2).",pf3=".round($row1[pf3v],2)." where bh='".$row[probh]."'");
 PointInto($userid,"交易成功，点评商品获得积分",$rowcontrol[pjjf]);
 PointUpdate($userid,$rowcontrol[pjjf]); 

 $sqlp="select avg(pf1) pf1v,avg(pf2) pf2v,avg(pf3) pf3v from epzhu_proyy where zt=0 and userid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");
 ...
 php_toheader("orderpjyy.php?orderbh=".$orderbh);

}
if(sqlzhuru($_POST[jvs])=="xgpj"){
 zwzr();
 $sj=date("Y-m-d H:i:s");
 $uip=$_SERVER["REMOTE_ADDR"];
 if(panduan("*","epzhu_tp where bh='".$orderbh."'")==1){$iftp=1;}else{$iftp=0;}
 
 updatetable("epzhu_propjyy","pf1=".sqlzhuru($_POST[hpjjf1]).",pf2=".sqlzhuru($_POST[hpjjf2]).",pf3=".sqlzhuru($_POST[hpjjf3]).",txt='".sqlzhuru($_POST[s1])."',pjlx=".$_POST[Rpj].",iftp=".$iftp." where orderbh=".$orderbh);
 $sql1="select avg(pf1) as pf1v,avg(pf2) as pf2v,avg(pf3) as pf3v from epzhu_propjyy where probh='".$row[probh]."' and selluserid=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$res1=mysql_query($sql1);
 $row1=mysql_fetch_array($res1);
 updatetable("epzhu_proyy","pf1=".round($row1[pf1v],2).",pf2=".round($row1[pf2v],2).",pf3=".round($row1[pf3v],2)." where bh='".$row[probh]."'");
 PointInto($userid,"交易成功，点评商品获得积分",$rowcontrol[pjjf]);
...
function tj(){
 ...
 tjwait();
 f1.action="orderpjyy.php?orderbh=<?=$orderbh?>";
}
</script>
</head>
<body>
...
<div class="right">
 ...
 <li class="l1"><a href="orderyy.php">我的订单</a></li>
 </ul>
 <? include("orderv.php");?>
 ...
 <li class="l5">已上传<strong class="red" id="nowtpnum"><?=returncount("epzhu_tp where bh='".$orderbh."' and type1='评价'")?></strong>张图片<br> 【<a href="orderpjtp.php?orderbh=<?=$orderbh?>" class="feng" target="_blank">管理图片</a>】</li>
 <!--上传E-->
 </ul>
 <ul class="u2">
 <li class="l1">描述相符：</li>
...
<div class="pjbtn"><? tjbtnr("发表评价")?></div>
 ...
 <div class="orderv1 orderv0">
  ...
  评价：<?=$rowpj[txt]?><br>
  <? while1("*","epzhu_tp where bh='".$orderbh."' order by xh asc");while($row1=mysql_fetch_array($res1)){$tp="../".str_replace(".","-1.",$row1[tp]);?>
 ...
 <? }else{?>
  <form name="f1" method="post" onsubmit="return tj()">
 <input type="hidden" value="xgpj" name="jvs" />
 <input type="hidden" id="hpjjf1" name="hpjjf1" value="0" />
 <input type="hidden" id="hpjjf2" name="hpjjf2" value="0" />
 <input type="hidden" id="hpjjf3" name="hpjjf3" value="0" />
 <div class="orderpj">
