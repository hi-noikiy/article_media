...
if($_POST[jvs]=="openshop"){
 ...
 if(panduan("uid,pwd","epzhu_user where uid='".$_SESSION[SHOPUSER]."' and pwd='".$t1."'")==0){Audit_alert("登录密码验证失败，返回重试！","openshop2.php");}
 $d=preg_split("/xcf/",$_POST[d1]);
 while1("*","epzhu_openyue where id=".$d[2]);if(!$row1=mysql_fetch_array($res1)){Audit_alert("操作失败，返回重试！","openshop2.php");}
