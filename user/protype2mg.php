...
$bh=$_GET[bh];
while0("*","epzhu_protypemg where userid=".$userid." and admin=2 and bh='".$bh."'");
if(!$row=mysql_fetch_array($res)){Audit_alert("来源错误","protypelistmg.php","parent.");}

if($_GET[control]=="update"){
 zwzr();
 if(panduan("*","epzhu_protypemg where admin=2 and name1='".sqlzhuru($_POST[tname1])."' and name2='".sqlzhuru($_POST[tname2])."' and id<>".$row[id])==1)
 {Audit_alert("该分组已存在！","protype2mg.php?bh=".$bh);}
 updatetable("epzhu_protypemg","name2='".sqlzhuru($_POST[tname2])."',xh=".sqlzhuru($_POST[txh]).",zt=0 where id=".$row[id]);
 php_toheader("protype2mg.php?t=suc&bh=".$bh);
}
?>
...
f1.action="protype2mg.php?control=update&bh=<?=$bh?>";
}
<? if($_GET["t"]=="suc"){?>
