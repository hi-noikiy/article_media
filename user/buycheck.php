...
for($i=0;$i<count($c);$i++){
 ...
 while0("*","epzhu_car where userid=".$rowuser[id]." and id=".$id."");if(!$row=mysql_fetch_array($res)){php_toheader("car.php");}
 
 while1("*","epzhu_pro where bh='".$row[probh]."' and zt=0 and ifxj=0");if(!$row1=mysql_fetch_array($res1)){Audit_alert("商品已下架或未审核","car.php");}
 ...
 if(empty($row[tcid])){$zm=$money;$tcv="";$tcfhxs=99;}
  ...
  while2("*","epzhu_taocan where id=".$row[tcid]);if(!$row2=mysql_fetch_array($res2)){Audit_alert("套餐已下架，请联系客服","car.php");}
  $zm=$row2[money1];
  $tcfhxs=$row2[fhxs];
 ...
 updatetable("epzhu_car","num=".$num.",money1=".$zm.",tcv='".$tcv."',shdz='".$shdz."',yunfei=".$yf.",tcfhxs=".$tcfhxs." where id=".$id);
