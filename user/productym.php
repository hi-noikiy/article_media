...
$userid=$rowuser[id];
$bh=$_GET[bh];
while0("*","epzhu_proym where bh='".$bh."' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("productlistym.php");}
...
if(empty($anzhuang)){
 ...
 if(!empty($anzhuang) && !is_numeric($anzhuang)){Audit_alert("安装费请输入数字","productlistym.php");}
 ...
 if(1==$ifuserjd){
 deletetable("epzhu_prouserdjym where probh='".$bh."'");
  for($i=1;$i<intval($_POST[djnum]);$i++){
  $zhekou=$_POST["zhekou_".$i];
  $djname=$_POST["name1_".$i];
  if(!empty($zhekou)){intotable("epzhu_prouserdjym","probh,userid,djname,admin,zhi","'".$bh."',".$row[userid].",'".$djname."',1,".$zhekou."");}
  }
 }

 updatetable("epzhu_proym",$ses."
 ...
 if(3==$fhxs){
  ...
  delFile($lj.$row[upf]);
  updatetable("epzhu_proym","upf='".$mc."' where bh='".$bh."' and userid=".$userid);
  }
 }
 //上传E
 //卡密B
 ...
 php_toheader("prosucym.php?bh=".$bh."&id=".$row[id]);
...
tjwait();
f1.action="productym.php?bh=<?=$bh?>&control=update";
}
function bzonc(x){
...
location.href="protypelistym.php";
}
...
<div class="right">
 ...
 [<a href="productlxym.php?action=update&id=<?=$row[id]?>">修改</a>]
 </li>
 <li class="l1">自定义分组：</li>
 <li class="l2">
 <select name="mty" class="inp1">
 <option value="0yjcode0">选择分组</option>
 <? while1("*","epzhu_protypeym where admin=1 and zt=0 and userid=".$luserid);while($row1=mysql_fetch_array($res1)){?>
 ...
 <? while2("*","epzhu_protypeym where admin=2 and name1='".$row1[name1]."' and zt=0 and userid=".$luserid);while($row2=mysql_fetch_array($res2)){?>
...
<!-- <div class="rts" style="cursor:pointer;" onclick="xtinfonc()">【点击<span id="xtzs" class="red">收起</span>】 选填部分可以不填写，但完善选填信息有助于您的商品更快出售，因此我们也建议您能耐心完善好。</div>-->
 ...
 <? $i=1;while1("*","epzhu_typesxym where admin=1 and typeid=".$row[ty1id]." order by xh asc");while($row1=mysql_fetch_array($res1)){?>
 <input type="hidden" value="<?=$row1[id]?>" name="tysxty1_<?=$i?>" />
 <ul class="uk1">
 <li class="l1"><?=$row1[name1]?>：</li>
 <li class="l2">
 <? $j=1;while2("*","epzhu_typesxym where admin=2 and name1='".$row1[name1]."' and typeid=".$row1[typeid]." order by xh asc");while($row2=mysql_fetch_array($res2)){?>
...
<ul class="uk"> 
 ...
 while2("*","epzhu_prouserdjym where probh='".$bh."' and djname='".$row1[name1]."'");if($row2=mysql_fetch_array($res2)){$zhekou=$row2[zhi];}else{$zhekou=$row1[zhekou];}
 ?>
 <ul class="dju2">
...
<li class="l1">出售域名：</li>
 <li class="l2"><input type="text"size="60" value="<?=$row[ysweb]?>" class="inp" name="tysweb" /></li>
<li class="l1">是否可转发：</li>
 <li class="l2">
 <label><input name="Rbz" type="radio" value="0" onclick="bzonc(5)" <? if(0==$row[bz]){?>checked="checked"<? }?> /> 可以</label>
 <label><input name="Rbz" type="radio" value="1" onclick="bzonc(4)" <? if(1==$row[bz]){?>checked="checked"<? }?> /> 不可以</label> 
 </li>
 <li class="l1">展示模板：</li>
 <li class="l2">
 ...
 <ul class="uk">
 <li class="l3"></li>
 <li class="l4"><? tjbtnr("提交","productlistym.php")?></li>
 </ul>
 </form>
 <!--E-->
</div> 
