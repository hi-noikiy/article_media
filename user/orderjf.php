...
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_order where orderbh='".$orderbh."' and ddzt='backerr' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("order.php");}

if(sqlzhuru($_POST[jvs])=="jf"){
 zwzr();
 $txt=sqlzhuru1($_POST[content]);
 if(empty($txt)){Audit_alert("申请理由内容不得为空，返回重试！","orderjf.php?orderbh=".$orderbh);}
 intotable("epzhu_orderlog","orderbh,userid,selluserid,admin,txt,sj","'".$orderbh."',".$row[userid].",".$row[selluserid].",1,'".$txt."','".$sj."'");
 updatetable("epzhu_order","ddzt='jf' where orderbh='".$orderbh."'");
 deletetable("epzhu_db where orderbh='".$orderbh."' and userid=".$userid);
 deletetable("epzhu_tk where orderbh='".$orderbh."' and userid=".$userid);
