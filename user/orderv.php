 ...
 <div class="orderv1">
  ...
  <div class="ztcontrol">
   ...
   <div class="d2">
    ...
    <div class="ds2"><strong>提醒：</strong>如果卖家长时间未发货，您可以【<a href="orderclose.php?orderbh=<?=$orderbh?>">取消订单</a>】。卖家也挺不容易，您可以【<a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$rows1[uqq]?>&site=<?=weburl?>&menu=yes" class="s2" target="_blank">提醒卖家尽快发货</a>】。</div>
    <div class="ds3">
    <a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$rows1[uqq]?>&site=<?=weburl?>&menu=yes" class="a1" target="_blank">提醒卖家尽快发货</a>
    <a href="orderclose.php?orderbh=<?=$orderbh?>" class="a0">取消订单</a>
    </div>
    <? }?>
    ...
    <div class="ds2"><strong>提醒：</strong>如果还需要该商品，请点<a href="<?=$au?>" target="_blank">再次购买</a>。</div>
    <? if(panduan("orderbh,userid","epzhu_propj where orderbh='".$orderbh."' and userid=".$userid)==0){?>
    ...
    <? 
	if($row[ddzt]=="backerr"){
    while1("*","epzhu_db where orderbh='".$orderbh."'");$row1=mysql_fetch_array($res1);
    ...
    <? 
	if($row[ddzt]=="back"){
    while1("*","epzhu_tk where orderbh='".$orderbh."'");$row1=mysql_fetch_array($res1);
    ...
    <div class="ds2"><strong>提醒：</strong>如果商品本身及卖家方面都无问题，你也还需要该商品，那么你可以<a href="orderqxtk.php?orderbh=<?=$orderbh?>">取消退款申请</a>。<br>你在 <?=$row[tksj]?> 申请了退款<br>你的退款理由：<?=$row[tkly]?></div>
    <div class="ds3">
