...
//函数B
if($_GET[action]=="del"){
deletetable("epzhu_carym where id=".$_GET[id]." and userid=".$rowuser[id]);
php_toheader("carym.php");
}elseif($_GET[action]=="dall"){
deletetable("epzhu_carym where userid=".$rowuser[id]);
echo "ok";exit;
}
//函数E
?>
...
if(shd==0){layer.alert("请先选择收货地址", {icon:5});return false;}
 ...
 if(buystr!=""){
  ...
  $.post("buyformym.php",{bv:buystr,cid:carid},function(result){
   if(result=="ok"){location.href="carpayym.php?carid="+carid;}
   else{layer.alert('提交失败，请重试', {icon:5});return false;}
  });
 }else{
  location.href="carpayym.php?carid="+carid;
 }
}
...
layer.open({
 ...
 $.get("carym.php",{action:"dall"},function(result){
  if(result=="ok"){location.href="carym.php";}
...
function yunfeicha(x,a,b){
 if(b==0){carmoney(x);}
  ...
  $.get("../tem/getyfym.php",{u:a,s:<?=$shdz?>,sl:inps,p:b},function(result){
...
<div id="shdzmain" style="display:none;">
 ...
 <li class="l2"><a href="carym.php?shdz=<?=$row1[id]?>" class="feng">选择</a> | <a href="javascript:void(0);" onclick="ediadd('<?=$row1[bh]?>')" class="feng">修改</a></li>
 </ul>
 <? $i++;}?>
 <div class="adddz"><a href="javascript:void(0);" onclick="addshdz()">添加收货地址</a></div>
</div>
<? if(!empty($shdzid)){updatetable("epzhu_carym","shdzid=".$shdzid." where userid=".$rowuser[id]);}?>
...
<?
$i=1;
while0("*","epzhu_carym where userid=".$rowuser[id]." group by selluserid order by sj desc");while($row=mysql_fetch_array($res)){
...
<div class="cartlist">
<?
while1("*","epzhu_carym where userid=".$rowuser[id]." and selluserid=".$row[selluserid]." order by sj desc");while($row1=mysql_fetch_array($res1)){
$tp="../".returntp("bh='".$row1[probh]."' order by iffm desc","-2");
while2("*","epzhu_proym where bh='".$row1[probh]."' and zt=0 and ifxj=0");if($row2=mysql_fetch_array($res2)){
$money=returnyhmoney($row2[yhxs],$row2[money2],$row2[money3],$sj,$row2[yhsj1],$row2[yhsj2],$row2[id]);
$money1=$row2["money1"];
$au="../productym/view".$row2[id].".html";
?>
<ul class="u1">
...
if(!empty($row2[ifuserdj])){
 ...
 $sqlu4="select * from epzhu_prouserdjym where probh='".$row2[bh]."' and djname='".$djname."'";mysql_query("SET NAMES 'GBK'");$resu4=mysql_query($sqlu4);
...
<li class="l8"><a href="carym.php?action=del&id=<?=$row1[id]?>">删除</a></li>
</ul>
<!--购买模板B-->
<?
$sqlt1="select * from epzhu_typeym where admin=2 and id=".$row2[ty2id];mysql_query("SET NAMES 'GBK'");$rest1=mysql_query($sqlt1);if($rowt1=mysql_fetch_array($rest1)){
