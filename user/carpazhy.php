<?
include("../config/conn.php");//二次--开发---联系QQ:120036745
include("../config/functionzh.php");//二--次-开发--联系QQ:120--0367--45
sesCheck();
...
$sj=date("Y-m-d H:i:s");
include("buycheckzh.php");

//B
if(sqlzhuru($_POST[jvs])=="carpay" && sqlzhuru($_POST[R1])=="alipay"){
if($needmoney<=$usermoney){Audit_alert("您的可用余额充足，请用余额直接支付。","carpayzh.php?carid=".$carid);}
zwzr();
$bh=time();
...
$return_url = weburl."user/orderzh.php";//页面跳转同步通知页面路径
...
echo $html_text;exit;
 ...
 if($needmoney>$usermoney){Audit_alert("您的可用余额不足，返回重试。","carpayzh.php?carid=".$carid);}
 zwzr();
 include("buyzh.php");
 php_toheader("orderzh.php");
...
function carpaytj(x){
 ...
 tjwait();
 if(rv=="alipay" || rv==""){fu="carpayzh.php?carid="+x;}
 else if(rv=="tenpay"){fu="tenpay/buy_indexzh.php?carid="+x;}
...
<div class="right">
 ...
 <div class="carpay">
  ...
  <li class="l3">
  <a href="orderzh.php">完成付款</a>
  <a href="javascript:void(0);" onClick="javascript:alert('付款遇到问题？请联系客服');location.href='carpayzh.php?carid=<?=$carid?>';return false;">付款遇到问题</a>
  </li>
  </ul>
  </div>
