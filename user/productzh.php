...
$userid=$rowuser[id];
$bh=$_GET[bh];
while0("*","epzhu_prozh where bh='".$bh."' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("productlistzh.php");}
...
if(empty($anzhuang)){
 ...
 if($money1<0 || $money2<0){Audit_alert("价格不能为负数！","productlistzh.php");}
 $fhxs=intval(sqlzhuru($_POST[Rfhxs]));
 if($rowcontrol[ifproduct]=="on"){$nzt=0;}else{$nzt=1;}
 if(!empty($anzhuang) && !is_numeric($anzhuang)){Audit_alert("安装费请输入数字","productlistzh.php");}
 ...
 if(1==$ifuserjd){
 deletetable("epzhu_prouserdjzh where probh='".$bh."'");
  for($i=1;$i<intval($_POST[djnum]);$i++){
  $zhekou=$_POST["zhekou_".$i];
  $djname=$_POST["name1_".$i];
  if(!empty($zhekou)){intotable("epzhu_prouserdjzh","probh,userid,djname,admin,zhi","'".$bh."',".$row[userid].",'".$djname."',1,".$zhekou."");}
  }
 }

 updatetable("epzhu_prozh",$ses."
 ...
 if(3==$fhxs){
  ...
  delFile($lj.$row[upf]);
  updatetable("epzhu_prozh","upf='".$mc."' where bh='".$bh."' and userid=".$userid);
  }
 }
 //上传E
 //卡密B
 ...
 php_toheader("prosuczh.php?bh=".$bh."&id=".$row[id]);
...
tjwait();
f1.action="productzh.php?bh=<?=$bh?>&control=update";
}
function bzonc(x){
...
location.href="protypelistzh.php";
}
...
<div class="right">
 ...
 [<a href="productlxzh.php?action=update&id=<?=$row[id]?>">修改</a>]
 </li>
 <li class="l1">自定义分组：</li>
 <li class="l2">
 <select name="mty" class="inp1">
 <option value="0yjcode0">选择分组</option>
 <? while1("*","epzhu_protypezh where admin=1 and zt=0 and userid=".$luserid);while($row1=mysql_fetch_array($res1)){?>
 ...
 <? while2("*","epzhu_protypezh where admin=2 and name1='".$row1[name1]."' and zt=0 and userid=".$luserid);while($row2=mysql_fetch_array($res2)){?>
 ...
 <label><input name="Rfhxs" type="radio" value="2" onclick="fhxsonc(2)" <? if(2==$row[fhxs]){?>checked="checked"<? }?> /> 网盘发货</label> &nbsp;&nbsp;
 <? }?>-->
 ...
 <label><input name="Rfhxs" type="radio" value="4" onclick="fhxsonc(4)" <? if(4==$row[fhxs]){?>checked="checked"<? }?> /> 自动发货</label>&nbsp;&nbsp;
 <? }?>
 ...
 <? $i=1;while1("*","epzhu_typesxzh where admin=1 and typeid=".$row[ty1id]." order by xh asc");while($row1=mysql_fetch_array($res1)){?>
 <input type="hidden" value="<?=$row1[id]?>" name="tysxty1_<?=$i?>" />
 <ul class="uk1">
 <li class="l1"><?=$row1[name1]?>：</li>
 <li class="l2">
 <? $j=1;while2("*","epzhu_typesxzh where admin=2 and name1='".$row1[name1]."' and typeid=".$row1[typeid]." order by xh asc");while($row2=mysql_fetch_array($res2)){?>
...
<div class="layui-form-item">
 ...
 while2("*","epzhu_prouserdjzh where probh='".$bh."' and djname='".$row1[name1]."'");if($row2=mysql_fetch_array($res2)){$zhekou=$row2[zhi];}else{$zhekou=$row1[zhekou];}
 ?>
 <ul class="dju2">
...
<!-- <li class="l1">出售网址：</li>
 ...
 <li class="l1">运营模式：</li>
 <li class="l2">
 <label><input name="Rbz" type="radio" value="0" onclick="bzonc(5)" <? if(0==$row[bz]){?>checked="checked"<? }?> /> 个人运营</label>
 <label><input name="Rbz" type="radio" value="1" onclick="bzonc(4)" <? if(1==$row[bz]){?>checked="checked"<? }?> /> 团队运营</label> 
 </li>
 <li class="l1">展示模板：</li>
 <li class="l2">
 ...
 <ul class="uk">
 <li class="l3"></li>
 <li class="l4"><? tjbtnr("提交","productlistzh.php")?></li>
 </ul>
 </form>
 <!--E-->
</div> 
