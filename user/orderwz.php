...
<div class="right">
 ...
 <li class="ltj"><input type="button" onclick="psel()" value="搜索" /> <input type="button" onclick="gourl('orderwz.php')" value="重置" /></li>
 </ul>
 <!--搜索E-->
 ...
 <!--列表开始-->
 <?
 pagef($ses,10,"epzhu_orderwz","order by sj desc");while($row=mysql_fetch_array($res)){
 $tp="../".returntp("bh='".$row[probh]."' order by iffm desc","-2");
 $cz="";
 if($row[ddzt]=="suc"){ //交易成功
  if(panduan("userid,orderbh","epzhu_propjwz where orderbh='".$row[orderbh]."' and userid=".$userid)==0){
  $cz="<a href='orderpjwz.php?orderbh=".$row[orderbh]."' class='btn feng'>评价</a>";
  }else{
  //开始判断是否是默认评价
  $sqlpj="select * from epzhu_propjwz where orderbh='".$row[orderbh]."' and userid=".$userid;mysql_query("SET NAMES 'GBK'");$respj=mysql_query($sqlpj);
  $rowpj=mysql_fetch_array($respj);
  if($rowpj['txt']!='暂未评价'){
	$cz="<a href='orderpjwz.php?orderbh=".$row[orderbh]."' class='blue'>已评价</a><br>";
  }else{
	$cz="<a href='orderpjwz.php?orderbh=".$row[orderbh]."' class='btn feng'>评价</a>";
  }
  }
  $cz=$cz."<a href='../productwz/view".returnproid($row[probh]).".html' target=_blank>再次购买</a>";
  ...
  $cz=$cz."<br><a href='orderclosewz.php?orderbh=".$row[orderbh]."' class='hui'>取消订单</a>";
 
 }elseif($row[ddzt]=="backsuc"){ //退款成功
  $cz="<a href='../productwz/view".returnproid($row[probh]).".html' target=_blank>重新购买</a>";
 
 }elseif($row[ddzt]=="backerr"){ //退款失败，不同意退款
  $cz="<a href='shouhuowz.php?orderbh=".$row[orderbh]."' class='btn'>收货</a>";
  $cz=$cz."<br><a href='ordertkwz.php?orderbh=".$row[orderbh]."' class='blue'>再次申请退款</a>";
  $cz=$cz."<br><a href='orderjfwz.php?orderbh=".$row[orderbh]."' class='hui'>申请客服介入</a>";

 }elseif($row[ddzt]=="db"){ //担保中
  $cz="<a href='shouhuowz.php?orderbh=".$row[orderbh]."' class='btn'>收货</a>";
  $cz=$cz."<br><a href='ordertkwz.php?orderbh=".$row[orderbh]."' class='hui'>退款</a>";

 }elseif($row[ddzt]=="close"){ //订单取消
  $cz="<a href='../productwz/view".returnproid($row[probh]).".html' target=_blank>重新购买</a>";

 }elseif($row[ddzt]=="jf"){ //纠纷处理中
  $cz="<a href='orderjf1wz.php?orderbh=".$row[orderbh]."' class='btn'>沟通</a>";
 ...
 <li class="l3"><a href="orderviewwz.php?orderbh=<?=$row[orderbh]?>" class="lookdd">查看订单</a></li>
 </ul>
 <ul class="typelist">
 <li class="l1">
 <a href="orderviewwz.php?orderbh=<?=$row[orderbh]?>"><img class="tp" src="<?=returntppd($tp,"img/none60x60.gif")?>" width="50" height="50" align="left" /></a>
 <a title="<?=$row["tit"]?>" href="orderviewwz.php?orderbh=<?=$row[orderbh]?>" class="a1"><?=returntitdian($row["tit"],100)?></a><br>
 </li>
 <li class="l0"></li>
