...
$userid=$rowuser[id];
while0("*","epzhu_shopmenu where userid=".$userid." and bh='".$bh."' and admin=2");if(!$row=mysql_fetch_array($res)){Audit_alert("来源错误","shopmenulist.php","parent.");}

if($_GET[control]=="update"){
 zwzr();
 if(panduan("*","epzhu_shopmenu where admin=2 and tit1='".sqlzhuru($_POST[ttit1])."' and tit2='".sqlzhuru($_POST[ttit2])."' and userid=".$userid." and id<>".$row[id])==1)
 {Audit_alert("该导航已存在！","shopmenu2.php?bh=".$bh);}
 updatetable("epzhu_shopmenu","tit2='".sqlzhuru($_POST[ttit2])."',aurl='".sqlzhuru($_POST[taurl])."',xh=".sqlzhuru($_POST[txh]).",targ=".sqlzhuru($_POST[R1]).",zt=0 where userid=".$userid." and id=".$row[id]);
