...
function psel(){
 ...
 location.href="sellorderkf.php?"+str;
}
</script>
</head>
<body>
...
<div class="right">
 ...
 <li class="ltj"><input type="button" onclick="psel()" value="搜索" /> <input type="button" onclick="gourl('sellorderkf.php')" value="重置" /></li>
 </ul>
 <!--搜索E-->
 ...
 <li class="l21">
 <?
 $sqlq="select sum(money1*num+yunfei) as orderall from epzhu_orderkf".$sestj." and ddzt='suc'";mysql_query("SET NAMES 'GBK'");$resq=mysql_query($sqlq);$rowq=mysql_fetch_array($resq);
 $money1=sprintf("%.2f",$rowq[orderall]);
 ?>
 交易成功（<strong class="green"><?=$money1?>元</strong>）&nbsp;&nbsp;&nbsp;&nbsp;
 <?
 $sqlq="select sum(money1*num+yunfei) as orderall from epzhu_orderkf".$sestj." and ddzt='db'";mysql_query("SET NAMES 'GBK'");$resq=mysql_query($sqlq);$rowq=mysql_fetch_array($resq);
 $money1=sprintf("%.2f",$rowq[orderall]);
 ?>
 正在担保（<strong class="blue"><?=$money1?>元</strong>）&nbsp;&nbsp;&nbsp;&nbsp;
 <?
 $sqlq="select sum(money1*num+yunfei) as orderall from epzhu_orderkf".$sestj." and ddzt='back'";mysql_query("SET NAMES 'GBK'");$resq=mysql_query($sqlq);$rowq=mysql_fetch_array($resq);
 $money1=sprintf("%.2f",$rowq[orderall]);
 ?>
 ...
 $au="sellorderviewkf.php?orderbh=".$row[orderbh];
 $tp="../".returntp("bh='".$row[probh]."' order by iffm desc","-2");
 $cz="";
 if($row[ddzt]=="suc"){ //交易成功
 
 }elseif($row[ddzt]=="wait"){ //等待发货
 $cz="<a href='fahuokf.php?orderbh=".$row[orderbh]."' class='btn'>发货</a>";
 $cz=$cz."<br><a href='sellclosekf.php?orderbh=".$row[orderbh]."' class='hui'>取消订单</a>";
 
 }elseif($row[ddzt]=="back"){ //退款处理中
 $cz="<a href='selltkkf.php?orderbh=".$row[orderbh]."' class='hui'>处理退款</a>";
 ...
 $cz="<a href='orderjf2kf.php?orderbh=".$row[orderbh]."' class='btn'>沟通</a>";
