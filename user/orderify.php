...
while1("*","epzhu_order where orderbh='".$bh."' and userid=".$userid);if(!$row1=mysql_fetch_array($res1)){echo "1";exit;}
$sj=date("Y-m-d H:i:s");
...
if (!empty($_FILES) && sqlzhuru($_POST['token']) == $verifyToken) {
	...
	if (in_array(strtolower($fileParts['extension']),$fileTypes)) {
		...
		intotable("epzhu_tp","bh,tp,type1,sj,userid,xh","'".$bh."','".$wjv."','评价','".$sj."',".$userid.",".$nxh."");
		echo '1';
	} else {
