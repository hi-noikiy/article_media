...
$sj=date("Y-m-d H:i:s");
while0("*","epzhu_orderyy where orderbh='".$orderbh."' and userid=".$userid);if(!$row=mysql_fetch_array($res)){php_toheader("orderyy.php");}

if(sqlzhuru($_POST[jvs])=="tk"){
 zwzr();
 $zfmm=sha1(sqlzhuru($_POST[t1]));
 if(panduan("uid,zfmm","epzhu_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("安全码有误！","ordertkyy.php?orderbh=".$orderbh);}
 if($row[ddzt]!="db" && $row[ddzt]!="backerr"){Audit_alert("未知错误！","orderviewyy.php?orderbh=".$orderbh);}
 $allmoney=$row[money1]*$row[num]+$row[yunfei];
 updatetable("epzhu_orderyy","ddzt='back',tksj='".$sj."',tkly='".sqlzhuru($_POST[t2])."' where userid=".$userid." and id=".$row[id]);
 ...
 //通知B
 $sqluser="select id,mot,ifmot,email,ifemail from epzhu_user where id=".$row[selluserid];mysql_query("SET NAMES 'GBK'");$resuser=mysql_query($sqluser);if(!$rowuser=mysql_fetch_array($resuser)){Audit_alert("安全码有误！","ordertkyy.php?orderbh=".$orderbh);}
 ...
 //通知E
 php_toheader("orderviewyy.php?orderbh=".$orderbh); 
...
<div class="right">
 ...
 f1.action="ordertkyy.php?orderbh=<?=$orderbh?>";
 }
 </script>
 ...
 * 卖家也挺不容易，如果是商品存在问题，而卖家又能积极处理问题，您可以<a href="http://wpa.qq.com/msgrd?v=3&uin=<?=returnqq($row[selluserid])?>&site=<?=weburl?>&menu=yes" target=_blank class="blue">与卖家再协商下</a>。
 ...
 <li class="l2">请输入您的安全码：(<a href="zfmmyy.php" class="red">忘了安全码？</a>)</li>
