...
if(1==$rowcontrol[ifuc]){
 ...
 list($uid, $username, $password, $email) = uc_user_login(sqlzhuru($_POST[t1]), sqlzhuru($_POST[t2]));  //登录
  if($uid>0) {   //uc登录成功
   ...
   if(panduan("uid","epzhu_user where uid='".sqlzhuru($_POST[t1])."'")==0){
    ...
    $uid=sqlzhuru($_POST[t1]);
	$sql="select id,uid,zt from epzhu_user where uid='".$uid."'";mysql_query("SET NAMES 'GBK'");$res=mysql_query($sql,$conn);$row=mysql_fetch_array($res);
