...
parent.ptype5.location="../tem/protype5kf.php?type1id=<?=$type1id?>&type2id=<?=$type2id?>&type3id=<?=$type3id?>&type4id="+a;
...
<body>
 ...
 <!--begin-->
  ...
  <a href="javascript:void(0);" class="a1"><?=$type3name?> <img border="0" src="../img/jiandown.gif" width="7" height="4" /></a>
   <?
   while0("*","epzhu_typekf where type1='".$type1name."' and type2='".$type2name."' and type3='".$type3name."' and admin=4 order by xh asc");while($row=mysql_fetch_array($res)){
   ?>
