...
if(sqlzhuru($_POST[jvs])=="getpwd"){
 ...
 while0("id,uid,email","epzhu_user where email='".$m."' and uid='".$uid."'");if(!$row=mysql_fetch_array($res)){Audit_alert("帐号与邮箱不匹配！","getpasswd.php");}

 //SMSMAIL入库
 if($rowcontrol[mailname]!="" && $rowcontrol[mailpwd]!="" && $rowcontrol[mailsmtp]!=""){
  $rnd=MakePass(6);
  updatetable("epzhu_user","getpwd='".$rnd."' where id=".$row[id]);
