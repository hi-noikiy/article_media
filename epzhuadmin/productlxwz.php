...
if($_GET[control]=="add"){
 ...
 while1("uid,zt,shopzt","epzhu_user where uid='".$uid."' and zt=1 and shopzt=2");if(!$row1=mysql_fetch_array($res1)){Audit_alert("会员不存在！","productlxwz.php");}
 ...
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0101,")){Audit_alert("权限不够","default.php");}
 zwzr();
 updatetable("epzhu_prowz","ty1id=".sqlzhuru($_POST[type1id]).",ty2id=".sqlzhuru($_POST[type2id]).",ty3id=".sqlzhuru($_POST[type3id]).",ty4id=".sqlzhuru($_POST[type4id]).",ty5id=".sqlzhuru($_POST[type5id])." where id=".$_GET[id]);
...
<div class="right">
 ...
 <a href="productlistwz.php">返回列表</a>
 </div> 
 
 <!--begin-->
 <? if($_GET[action]==""){?>
 <form name="f1" method="post" onsubmit="return tjadd('productlxwz.php',1)">
