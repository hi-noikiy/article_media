...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","epzhu_typesxzh where admin=1 and name1='".sqlzhuru($_POST[t1])."' and typeid=".$typeid)==1){Audit_alert("该附加选项已存在！","typesxzh.php?typeid=".$typeid);}
 intotable("epzhu_typesxzh","typeid,name1,sj,xh,admin,ifjd,ifzi,ifsel","".$typeid.",'".sqlzhuru($_POST[t1])."','".$sj."',".sqlzhuru($_POST[t2]).",1,".sqlzhuru($_POST[Rifjd]).",".sqlzhuru($_POST[Rifzi]).",".sqlzhuru($_POST[Rifsel])."");
 php_toheader("typesxzh.php?t=suc&typeid=".$typeid);
 
}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 $id=$_GET[id];
 if(panduan("*","epzhu_typesxzh where admin=1 and name1='".sqlzhuru($_POST[t1])."' and id<>".$id." and typeid=".$typeid)==1){Audit_alert("该附加选项已存在！","typesxzh.php?action=update&id=".$id."&typeid=".$typeid);}
 updatetable("epzhu_typesxzh","name1='".sqlzhuru($_POST[t1])."' where name1='".sqlzhuru($_POST[oldty1])."' and typeid=".$typeid);
 updatetable("epzhu_typesxzh","sj='".$sj."',xh=".sqlzhuru($_POST[t2]).",ifjd=".sqlzhuru($_POST[Rifjd]).",ifzi=".sqlzhuru($_POST[Rifzi]).",ifsel=".sqlzhuru($_POST[Rifsel])." where id=".$_GET[id]);
 php_toheader("typesxzh.php?t=suc&action=update&id=".$id."&typeid=".$typeid);
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","typesxzh.php?action=".$_GET[action]."&id=".$_GET[id]."&typeid=".$_GET[typeid]);}?>
 <div class="bqu1">
 ...
 f1.action="typesxzh.php?control=add&typeid=<?=$typeid?>";
 }
 </script>
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("epzhu_typesxzh"," and admin=1 and typeid=".$typeid)?>" /> <span class="fd">序号越小，越靠前</span></li>
 ...
 while0("*","epzhu_typesxzh where id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("typesxlistzh.php?typeid=".$typeid);}
 ?>
 <script language="javascript">
 function tj(){
 ...
 if((document.f1.t2.value).replace(/\s/,"")=="" || isNaN(document.f1.t2.value)){alert("请输入有效的排序号！");document.f1.t2.focus();return false;}
 layer.msg('正在提交', {icon: 16  ,time: 0,shade :0.25});
 f1.action="typesxzh.php?control=update&id=<?=$row[id]?>&typeid=<?=$typeid?>";
 }
 </script>
 <form name="f1" method="post" onsubmit="return tj()">
