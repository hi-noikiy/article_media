...
<div class="right">
 ...
 <ul class="ksedi">
 <li class="l2">
 <a href="typesxkf.php?typeid=<?=$typeid?>" class="a1">新增选项</a>
 <a href="javascript:checkDEL(18,'epzhu_typesxkf')" class="a2">删除</a>
 </li>
 </ul>
 <ul class="qjlistcap">
 ...
 <?
 while1("*","epzhu_typesxkf where admin=1 and typeid=".$typeid." order by xh asc");while($row1=mysql_fetch_array($res1)){
 $nu="typesxkf.php?action=update&id=".$row1[id]."&typeid=".$typeid;
 ?>
 <ul class="qjlist1">
 ...
 <li class="l5"><a href="typesx1kf.php?ty1id=<?=$row1[id]?>">添加二级选项</a><span></span><a href="<?=$nu?>">编辑</a></li>
 </ul>
 <?
 while2("*","epzhu_typesxkf where admin=2 and name1='".$row1[name1]."' and typeid=".$typeid." order by xh asc");while($row2=mysql_fetch_array($res2)){
 $nu="typesx1kf.php?action=update&id=".$row2[id]."&ty1id=".$row1[id]; 
 ?>
 <ul class="qjlist2">
