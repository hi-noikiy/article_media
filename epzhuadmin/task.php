...
$sqltask="select * from epzhu_task where id=".$id." and taskty=0";mysql_query("SET NAMES 'GBK'");$restask=mysql_query($sqltask);
if(!$rowtask=mysql_fetch_array($restask)){php_toheader("tasklist.php");}
$bh=$rowtask[bh];

if(!empty($rowtask[useridhf])){
$sqltaskhf="select * from epzhu_taskhf where bh='".$bh."' and taskty=0 and useridhf=".$rowtask[useridhf]."";mysql_query("SET NAMES 'GBK'");$restaskhf=mysql_query($sqltaskhf);
...
if($_GET[control]=="update" && $_POST[jvs]=="zt8"){
 ...
 if(8==$rowtask[zt]){ //纠纷
  ...
  if(0==$zt){
   ...
   updatetable("epzhu_task","zt=9 where id=".$id);
   $txt="判定雇主胜利";
   intotable("epzhu_tasklog","bh,userid,useridhf,admin,txt,sj,fj","'".$bh."',".$rowtask[userid].",".$rowtask[useridhf].",3,'".$txt."','".$sj."',''");
  }elseif(1==$zt){
   ...
   updatetable("epzhu_task","zt=5 where id=".$id);
   $txt="判定接手方胜利";
   intotable("epzhu_tasklog","bh,userid,useridhf,admin,txt,sj,fj","'".$bh."',".$rowtask[userid].",".$rowtask[useridhf].",3,'".$txt."','".$sj."',''");
  }
 ...
 if(1==$rowtask[zt]){
  ...
  updatetable("epzhu_task","zt=0,yxq='".$endsj."' where id=".$id);
  }
  elseif($zt==1){
   if($rowtask[money4]>0){
  ...
  updatetable("epzhu_task","zt=2 where id=".$id);
  }
 }
...
<div class="right">
 ...
 <? 
 while2("*","epzhu_user where id=".$rowtask[userid]);$row2=mysql_fetch_array($res2);
 ?>
 <ul class="viewul">
 <li class="l1">任务主题：</li>
