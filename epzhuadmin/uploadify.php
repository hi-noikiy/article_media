...
while1("*","epzhu_admin where adminuid='".sqlzhuru($_POST[adminuid])."' and adminpwd='".sqlzhuru($_POST[adminpwd])."'");if(!$row1=mysql_fetch_array($res1)){echo "1";exit;}
$adminqx=$row1[qx];
if(!strstr($adminqx,",0,") && !strstr($adminqx,",0102,")){echo "1";exit;}
$bh=sqlzhuru($_POST["bh"]);
while1("*","epzhu_pro where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){echo "1";exit;}
$userid=$row1[userid];
