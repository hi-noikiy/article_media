...
if(empty($_SESSION[SHOPADMIN])){Audit_alert("登录超时","un.php","parent.");}
$bh=$_GET[bh];
...
if($admin==1){ //商品B
 while1("*","epzhu_pro where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){Audit_alert("来源错误","./","parent.");}
 ...
 for($k=0; $k<$total; $k++) {
  if(returncount("epzhu_tp where bh='".$bh."'")<7){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
	...
	intotable("epzhu_tp","bh,tp,type1,sj,userid,xh","'".$bh."','".$wjv."','商品','".$sj."',".$row1[userid].",".$nxh."");
   }
  }
 }

}elseif($admin==6){ //资讯
 while1("*","epzhu_news where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){Audit_alert("来源错误","./","parent.");}
 ...
 for($k=0; $k<$total; $k++) {
  if(returncount("epzhu_tp where bh='".$bh."'")<1){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
	...
	$wjv=$targetFolder.$mbh.".jpg";
	$nxh=returnxh("epzhu_tp"," and bh='".$bh."'");
	intotable("epzhu_tp","bh,tp,type1,iffm,sj,userid,xh","'".$bh."','".$wjv."','资讯',1,'".$sj."',".returnjgdw($row1[userid],"",0).",".$nxh."");
   }
  }
 }
