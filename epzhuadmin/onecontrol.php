...
if($_GET[control]=="update"){
 ...
 $sj=date("Y-m-d H:i:s");
 if(panduan("tyid","epzhu_onecontrol where tyid=".$tyid)==1){
 updatetable("epzhu_onecontrol","sj='".$sj."',txt='".sqlzhuru1($_POST[content])."' where tyid=".$tyid);
 }else{
 intotable("epzhu_onecontrol","sj,tyid,txt","'".$sj."',".$tyid.",'".sqlzhuru1($_POST[content])."'");
 }
 php_toheader("onecontrol.php?t=suc&tyid=".$tyid);
}

while0("*","epzhu_onecontrol where tyid=".$tyid);$row=mysql_fetch_array($res);
