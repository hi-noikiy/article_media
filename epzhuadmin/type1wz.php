...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","epzhu_typewz where admin=1 and type1='".sqlzhuru($_POST[t1])."'")==1){Audit_alert("该分组已存在！","type1wz.php");}
 intotable("epzhu_typewz","admin,type1,xh,sj,col,iftj,tjmoney,sellbl,seotit,seokey,seodes,dbsj","1,'".sqlzhuru($_POST[t1])."',".sqlzhuru($_POST[t2]).",'".$sj."','".sqlzhuru($_POST[tcol])."',".sqlzhuru($_POST[R1]).",".abs($_POST[ttjmoney]).",".abs($_POST[tsellbl]).",'".sqlzhuru($_POST[tseotit])."','".sqlzhuru($_POST[tseokey])."','".sqlzhuru($_POST[tseodes])."',".sqlzhuru($_POST[tdbsj])."");$id=mysql_insert_id();
 ...
 php_toheader("type1wz.php?t=suc");
 
}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","epzhu_typewz where admin=1 and type1='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1)
 {Audit_alert("该分组已存在！","type1wz.php?action=update&id=".$_GET[id]);}
 updatetable("epzhu_typewz","type1='".sqlzhuru($_POST[t1])."' where type1='".sqlzhuru($_POST[oldty1])."'");
 updatetable("epzhu_typewz","
 ...
 php_toheader("type1wz.php?t=suc&action=update&id=".$_GET[id]);
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","type1wz.php?action=".$_GET[action]."&id=".$_GET[id]);}?>
 ...
 f1.action="type1wz.php?control=add";
 }
 </script>
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("epzhu_typewz"," and admin=1")?>" /> <span class="fd">序号越小，越靠前</span></li>
...
<? 
 ...
 while0("*","epzhu_typewz where admin=1 and id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("jiajctypelist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
 if((document.f1.t2.value).replace(/\s/,"")=="" || isNaN(document.f1.t2.value)){alert("请输入有效的排序号！");document.f1.t2.focus();return false;}
 layer.msg('正在提交', {icon: 16  ,time: 0,shade :0.25});
 f1.action="type1wz.php?control=update&id=<?=$row[id]?>";
 }
 function deltp(x){
  if(confirm("确定要删除该图标吗？")){location.href="type1wz.php?id=<?=$_GET[id]?>&control=del&tp="+x;}else{return false;}
 }
 </script>
 <form name="f1" method="post" onsubmit="return tj()" enctype="multipart/form-data">
