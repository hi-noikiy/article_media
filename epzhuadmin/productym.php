...
AdminSes_audit();
$bh=$_GET[bh];
while0("*","epzhu_proym where bh='".$bh."'");if(!$row=mysql_fetch_array($res)){php_toheader("productlistym.php");}
...
if($_GET[control]=="update"){
 ...
 if(3==$fhxs){
  ...
  delFile($lj.$row[upf]);
  updatetable("epzhu_proym","upf='".$mc."' where bh='".$bh."'");
  }
 }
 //上传E
 //卡密B
 ...
 php_toheader("productym.php?t=suc&bh=".$bh);
...
<div class="yjcode">
 <? $leftid=1;include("menu_productym.php");?>

<div class="right">

 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！[<a href='productlxym.php'>继续添加新商品</a>]","productym.php?bh=".$bh);}?>
 <? include("rightcap4kf.php");?>
 ...
 <li class="l21"><strong><?=returntype(1,$row[ty1id])." - ".returntype(2,$row[ty2id])." - ".returntype(3,$row[ty3id])." - ".returntype(4,$row[ty4id])." - ".returntype(5,$row[ty5id])?></strong> [<a href="productlxym.php?action=update&id=<?=$row[id]?>">修改</a>]</li>
