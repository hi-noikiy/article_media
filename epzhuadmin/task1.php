...
$id=$_GET[id];
$sj=date("Y-m-d H:i:s");
$sqltask="select * from epzhu_task where id=".$id." and taskty=1";mysql_query("SET NAMES 'GBK'");$restask=mysql_query($sqltask);
...
if($_GET[control]=="update" && $_POST[jvs]=="zt105"){
 if(105==$rowtask[zt]){
  ...
  updatetable("epzhu_task","zt=100,yxq='".$endsj."' where id=".$id);
  }
  elseif($zt==2){
   if($rowtask[money4]>0){
  ...
  updatetable("epzhu_task","zt=106 where id=".$id);
  }
 }
