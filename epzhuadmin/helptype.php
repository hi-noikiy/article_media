...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","epzhu_helptype where admin=1 and name1='".sqlzhuru($_POST[t1])."'")==1){Audit_alert("该帮助中心分类已存在！","helptype.php");}
 intotable("epzhu_helptype","name1,sj,xh,admin","'".sqlzhuru($_POST[t1])."','".$sj."',".sqlzhuru($_POST[t2]).",1");
 php_toheader("helptype.php?t=suc");
 
}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","epzhu_helptype where admin=1 and name1='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1){Audit_alert("该帮助中心分类已存在！","helptype.php?action=update&id=".$_GET[id]);}
 updatetable("epzhu_helptype","name1='".sqlzhuru($_POST[t1])."' where name1='".sqlzhuru($_POST[oldty1])."'");
 updatetable("epzhu_helptype","sj='".$sj."',xh=".sqlzhuru($_POST[t2])." where id=".$_GET[id]);
...
<div class="right">
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("epzhu_helptype"," and admin=1")?>" /> <span class="fd">序号越小，越靠前</span></li>
 ...
 while0("*","epzhu_helptype where id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("helptypelist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
