...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","epzhu_type where admin=5 and type1='".sqlzhuru($_POST[t100])."' and type2='".sqlzhuru($_POST[t99])."' and type3='".sqlzhuru($_POST[t98])."' and type4='".sqlzhuru($_POST[t97])."' and type5='".sqlzhuru($_POST[t1])."'")==1)
 {Audit_alert("该分组已存在！","type5.php?ty1id=".$ty1id."&ty2id=".$ty2id."&ty3id=".$ty3id."&ty4id=".$ty4id);}
 intotable("epzhu_type","admin,type1,type2,type3,type4,type5,xh,sj","5,'".sqlzhuru($_POST[t100])."','".sqlzhuru($_POST[t99])."','".sqlzhuru($_POST[t98])."','".sqlzhuru($_POST[t97])."','".sqlzhuru($_POST[t1])."',".sqlzhuru($_POST[t2]).",'".$sj."'");
 php_toheader("type5.php?t=suc&ty1id=".$ty1id."&ty2id=".$ty2id."&ty3id=".$ty3id."&ty4id=".$ty4id);
 
}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","epzhu_type where admin=5 and type1='".sqlzhuru($_POST[t100])."' and type2='".sqlzhuru($_POST[t99])."' and type3='".sqlzhuru($_POST[t98])."' and type4='".sqlzhuru($_POST[t97])."' and type5='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1)
 {Audit_alert("该分组已存在！","type5.php?action=update&id=".$_GET[id]."&ty1id=".$ty1id."&ty2id=".$ty2id."&ty3id=".$ty3id."&ty4id=".$ty4id);}
 updatetable("epzhu_type","type5='".sqlzhuru($_POST[t1])."' where type1='".$ty1name."' and type2='".$ty2name."' and type3='".$ty3name."' and type4='".$ty4name."' and type5='".sqlzhuru($_POST[oldty5])."'");
...
<div class="right">
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("epzhu_type"," and type1='".$ty1name."' and type2='".$ty2name."' and type3='".$ty3name."' and type4='".$ty4name."' and admin=5")?>" /> <span class="fd">序号越小，越靠前</span></li>
 ...
 while0("*","epzhu_type where admin=5 and id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("typelist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
