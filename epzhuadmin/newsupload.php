...
$bh=$_GET[bh];
while1("bh,sj","epzhu_news where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){exit;}
$tplj1="upload/news/".dateYMDN($row1[sj])."/".$bh."/";
 ...
 $nbh=time();
 $wjv=$tplj1.$nbh.".jpg";
 intotable("epzhu_clear","bh,tp,type1,sj","'".$bh."','".$wjv."','".$_GET[ty]."','".$sj."'");$clearid=mysql_insert_id();
 ...
 php_toheader("newsupload.php?bh=".$bh."&tpbh=".$nbh."&clearid=".$clearid);
  ...
  if(panduan("bh","epzhu_news where bh='".$bh."'")==1){
  delFile("../".$tplj1.$_GET[tpbh].".jpg");
  delFile("../".$tplj1.$_GET[tpbh]."-1.jpg");
  if($_GET[clearid]!=""){deletetable("epzhu_clear where id=".$_GET[clearid]);}
  if($_GET[tpid]!=""){deletetable("epzhu_tp where bh='".$bh."' and id=".$_GET[tpid]);}
