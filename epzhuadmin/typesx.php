...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","epzhu_typesx where admin=1 and name1='".sqlzhuru($_POST[t1])."' and typeid=".$typeid)==1){Audit_alert("该附加选项已存在！","typesx.php?typeid=".$typeid);}
 ...
 zwzr();
 $id=$_GET[id];
 if(panduan("*","epzhu_typesx where admin=1 and name1='".sqlzhuru($_POST[t1])."' and id<>".$id." and typeid=".$typeid)==1){Audit_alert("该附加选项已存在！","typesx.php?action=update&id=".$id."&typeid=".$typeid);}
...
<div class="right">
 ...
 while0("*","epzhu_typesx where id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("typesxlist.php?typeid=".$typeid);}
 ?>
 <script language="javascript">
 function tj(){
