...
AdminSes_audit();
$bh=$_GET[bh];
while0("*","epzhu_prowz where bh='".$bh."'");if(!$row=mysql_fetch_array($res)){php_toheader("productlistwz.php");}
...
if($_GET[control]=="update"){
 ...
 if(3==$fhxs){
  ...
  delFile($lj.$row[upf]);
  updatetable("epzhu_prowz","upf='".$mc."' where bh='".$bh."'");
  }
 }
 //上传E
 //卡密B
 ...
 php_toheader("productwz.php?t=suc&bh=".$bh);
...
<div class="yjcode">
 <? $leftid=1;include("menu_productwz.php");?>

<div class="right">

 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！[<a href='productlxwz.php'>继续添加新商品</a>]","productwz.php?bh=".$bh);}?>
 <? include("rightcap4wz.php");?>
 ...
 <li class="l21"><strong><?=returntype(1,$row[ty1id])." - ".returntype(2,$row[ty2id])." - ".returntype(3,$row[ty3id])." - ".returntype(4,$row[ty4id])." - ".returntype(5,$row[ty5id])?></strong> [<a href="productlxwz.php?action=update&id=<?=$row[id]?>">修改</a>]</li>
 <li class="l1">自定义分组：</li>
 <li class="l2">
 <select name="mty" class="inp">
 <option value="0yjcode0">选择分组</option>
 <? while1("*","epzhu_protypewz where admin=1 and zt=0 and userid=".$row[userid]);while($row1=mysql_fetch_array($res1)){?>
 ...
 <? while2("*","epzhu_protypewz where admin=2 and name1='".$row1[name1]."' and zt=0 and userid=".$row[userid]);while($row2=mysql_fetch_array($res2)){?>
